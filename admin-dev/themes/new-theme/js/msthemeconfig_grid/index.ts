...
// 2. initialize the grid component by providing grid id
  ...
  const escapeTextareaContent = function(value: unknown): string {
    return String(value ?? '')
      .replace(/&/g, '&amp;')
      .replace(/</g, '&lt;')
      .replace(/>/g, '&gt;');
  ...
  const offerFormTemplate = function (data: {
    ...
    weight: any;
    description: any;
    oi_offer_memo: string | null | undefined;
  } | undefined) {
    ...
    <div class="card-body">
      ...
      <input type="hidden" name="_token" value="${$('#offer-row-token').val()}"/>
      ...
      <input type="hidden" name="offer-new" value="${data.new}"/>
        ...
        <div class="form-group">
          ...
          <textarea class="autoload_rte form-control" id="offer-memo" name="offer-memo">${escapeTextareaContent(data.oi_offer_memo)}</textarea>
        </div>
        ...
        <div class="form-group">
          ...
          <textarea class="autoload_rte form-control" id="offer-message-short" name="offer-message-short">${escapeTextareaContent(data.description)}</textarea>
        </div>
      </form>
      </p>
    </div>
  ...
  $(document).on('click', '#offer-row-submit', function (e) {
    // Sync TinyMCE editor content back to textarea before reading values
    ...
    $.ajax({
      ...
      .done(function (data) {
        ...
        if (offer.new === true) {
          let deleteUrl = ($('#offer-delete-url-base').val() ?? '').toString().replace('TOKEN_ID', idProduct);
          let newRowHtml = `
          ...
          //descr
          $('#offer-row-table tr[data-row-id="' + idProduct + '"] td:eq(1)').html(`<div style="max-height: 150px;overflow: scroll">${offer.description}</div>`);
          //memo
