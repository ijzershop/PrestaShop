 ...
 * @since 1.5
 */
class HTMLTemplatePhysicalOrderSlip extends HTMLTemplate
{
    /**
     * @var Order
     */
    ...
    public function getHeader()
    {
        ...
        $this->smarty->assign(['header' => Context::getContext()->getTranslator()->trans('Balie Bon', [], 'Shop.Pdf')]);
    ...
    public function getContent()
    {
        ...
        if (isset($this->order->id_address_delivery) && $this->order->id_address_delivery) {
            ...
            $formatted_delivery_address = AddressFormat::generateAddress($delivery_address,
                $deliveryAddressPatternRules, '<br />', ' ');
        }
        ...
        foreach ($order_details as $id => &$order_detail) {
            $order_detail['product_name'] = AttributeGroup::stripSawCutModuleAttributeGroupName($order_detail['product_name'], Context::getContext()->language->id);

            $prod = new Product($order_detail['product_id']);

            $order_detail['description'] = $prod->description[Context::getContext()->language->id];
            $order_detail['description_short'] = $prod->description_short[Context::getContext()->language->id];
        ...
        foreach ($footer as $key => $value) {
            $footer[$key] = Tools::ps_round($value, Context::getContext()->getComputingPrecision(),
        ...
        if (!$legal_free_text) {
            $legal_free_text = Configuration::get('PS_INVOICE_LEGAL_FREE_TEXT',
                (int)Context::getContext()->language->id, null, (int)$this->order->id_shop);
        }
        ...
        $tpls = [
            'style_tab' => $this->smarty->fetch($this->getTemplate('physical-order-slip.style-tab')),
            'addresses_tab' => $this->smarty->fetch($this->getTemplate('physical-order-slip.addresses-tab')),
            'summary_tab' => $this->smarty->fetch($this->getTemplate('physical-order-slip.summary-tab')),
            'product_tab' => $this->smarty->fetch($this->getTemplate('physical-order-slip.product-tab')),
            'tax_tab' => $this->getTaxTabContent(),
            'payment_tab' => $this->smarty->fetch($this->getTemplate('physical-order-slip.payment-tab')),
            'note_tab' => $this->smarty->fetch($this->getTemplate('physical-order-slip.note-tab')),
            'total_tab' => $this->smarty->fetch($this->getTemplate('physical-order-slip.total-tab')),
    ...
    {
        return Configuration::get('PS_CREDIT_SLIP_PREFIX', Context::getContext()->language->id, null,
                $this->order->id_shop) . sprintf('%06d', $this->order->id) . '.pdf';
    }
}
