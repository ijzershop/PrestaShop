  ...
  public function initContent()
  {
    ...
    switch ($this->method) {
        case 'fetchbyreference':
          ...
          $acceptedOrderStatusIds = explode(",", Configuration::get('ADDTOORDER_ORDER_STATUSES', null, Context::getContext()->shop->id_shop_group, Context::getContext()->shop->id, "2,3,7,10,16"));
          $acceptedCarrierRefs = explode(",", Configuration::get('ADDTOORDER_ACCEPTED_CARRIERS', null, Context::getContext()->shop->id_shop_group, Context::getContext()->shop->id, ""));
          $addToOrderCarrierId = Carrier::getAddToOrderCarrierId();
          $availableOrders =[];
            foreach ($result as $key => $order) {

              $orderCarrier = new Carrier((int)$order->id_carrier);
              $orderCarrierRef = Validate::isLoadedObject($orderCarrier)
                  ? (int)$orderCarrier->id_reference
                  : (int)$order->id_carrier;

              $isCarrierAccepted = empty($acceptedCarrierRefs[0]) || in_array($orderCarrierRef, $acceptedCarrierRefs);

              if(in_array($order->current_state, $acceptedOrderStatusIds) &&
                  $orderCarrierRef !== $addToOrderCarrierId &&
                  $isCarrierAccepted){
