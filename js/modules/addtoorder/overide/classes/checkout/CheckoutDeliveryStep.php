...
class CheckoutDeliveryStep extends CheckoutDeliveryStepCore
{
    ...
    public function handleRequest(array $requestParams = array())
    {
        ...
        if (isset($requestParams['added_to_order'])) {
            ...
            $deliveryOption = $this->getCheckoutSession()->getSelectedDeliveryOption();
            $addToOrderCarrierId = Carrier::getAddToOrderCarrierId();

            // Only set added_to_order if the selected carrier is the add-to-order carrier
            $deliveryOptionCarrier = new Carrier((int)$deliveryOption);
            $deliveryOptionRef = Validate::isLoadedObject($deliveryOptionCarrier)
                ? (int)$deliveryOptionCarrier->id_reference
                : (int)$deliveryOption;
            if ($deliveryOption && $deliveryOptionRef == $addToOrderCarrierId) {
