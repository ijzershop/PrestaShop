...
class PrestaShopOverrider extends BridgeConnectorCore
{
    ...
    {
        if ($ssl && version_compare(_PS_VERSION_, '1.7', '<=')) {
    ...
    public function setImage($entity_type, $image_id, $img, $type)
    {
        ...
        foreach ($image_types as $image_type) {
            ...
            $generate_hight_dpi_images = version_compare(_PS_VERSION_, '8.0', '>=') ? false : (bool) Configuration::get('PS_HIGHT_DPI');
            $res_2x = false;
    ...
    public function getImageDir($type)
    {
        ...
        switch ($type) {
            case self::PRODUCT:
                $path = version_compare(_PS_VERSION_, '8.0', '>=') ? _PS_PRODUCT_IMG_DIR_ : _PS_PROD_IMG_DIR_;
                break;
            case self::CATEGORY:
    ...
    {
        return version_compare(_PS_VERSION_, '1.7', '>=') ? json_encode($arr) : Tools::jsonEncode($arr);
    }

    public function stripSlashes($str)
    {
        return version_compare(_PS_VERSION_, '8.0', '>=') ? stripslashes($str) : Tools::stripslashes($str);
    }
