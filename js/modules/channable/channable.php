...
class Channable extends Module
{
    ...
    public function __construct()
    {
        ...
        $this->ps_versions_compliancy = ['min' => '1.6.1.24', 'max' => _PS_VERSION_];
    ...
    PRIMARY KEY  (`id_channable_stock_update`)
        ...
        Db::getInstance()->execute('ALTER TABLE `' . _DB_PREFIX_ . 'channable_stock_update` ADD INDEX idx_prod_attr (id_product, id_product_attribute)');
    ...
    public function getContent()
    {
        ...
        if (Tools::getValue('submitChannableOrderSettingsModule') == '1') {
            ...
            if (Tools::getValue('enable_advanced_stock_fields') != '') {
                Configuration::updateValue('CHANNABLE_ADD_ADVANCED_STOCK', (int) Tools::getValue('enable_advanced_stock_fields'));
            }
            if (Tools::getValue('enable_use_submitted_tax_rate') != '') {
                Configuration::updateValue('CHANNABLE_USE_SUBMITTED_TAX_RATE', (int) Tools::getValue('enable_use_submitted_tax_rate'));
            }
        ...
        if (Tools::getValue('submitChannableMarketplaceIDAssignmentModule') == '1') {
            if (Tools::getValue('msaid')) {
                if (is_array(Tools::getValue('msaid'))) {
                    Configuration::updateValue('CHANNABLE_MARKETPLACE_ID_ASSIGNMENTS', json_encode(Tools::getValue('msaid')));
                }
            }
            $this->context->smarty->assign('success_message', $this->l('Assigned shipping status to marketplace updated'));
        }

        if (Tools::getValue('submitChannableCarrierShippingMethodAssignmentModule') == '1') {
            if (Tools::getValue('csma')) {
                if (is_array(Tools::getValue('csma'))) {
                    Configuration::updateValue('CHANNABLE_CARRIER_SHIPPINGMETHOD_ASSIGNMENTS', json_encode(Tools::getValue('csma')));
                }
            }
            $this->context->smarty->assign('success_message', $this->l('Assigned carriers based on shipping method updated'));
        }

        if (Tools::getValue('submitChannableCountryCarrierShippingMethodAssignmentModule') == '1') {
            if (Tools::getValue('ccma')) {
                if (is_array(Tools::getValue('ccma'))) {
                    Configuration::updateValue('CHANNABLE_COUNTRY_SHIPPINGMETHOD_ASSIGNMENTS', json_encode(Tools::getValue('ccma')));
                }
            }
            $this->context->smarty->assign('success_message', $this->l('Assigned carriers based on country updated'));
        }
        ...
        $this->context->smarty->assign('product_stock_update_cron_url', $this->context->link->getModuleLink('channable', 'cron', ['sendProductUpdates' => '1']));
        ...
        $this->context->smarty->assign('marketplace_id_assignments', self::getMarketplaceIDAssignments());
        ...
        $this->context->smarty->assign('carrier_shippingmethod_assignments', self::getCarrierShippingMethodAssignments());
        $this->context->smarty->assign('country_shippingmethod_assignments', self::getCountryShippingMethodAssignments());
        ...
        return [
            'form' => [
                ...
                'input' => [
                    ...
                    [
                        'type' => 'switch',
                        'desc' => $this->l('If active, product stock updates will be triggered when shop backend is used.'),
                        'name' => 'CHANNABLE_DO_CRON_FROM_BACKEND',
                        'label' => $this->l('Send product stock updates while in Backend'),
                        'is_bool' => true,
                        'values' => [
                            [
                                'id' => 'send_cron_backend_active_on',
                                'value' => true,
                                'label' => $this->l('Enabled'),
                            ],
                            [
                                'id' => 'send_cron_backend_active_off',
                                'value' => false,
                                'label' => $this->l('Disabled'),
                            ],
                        ],
    ...
    protected function getConfigFormValues()
    {
        return [
            ...
            'CHANNABLE_DO_CRON_FROM_BACKEND' => Tools::getValue('CHANNABLE_DO_CRON_FROM_BACKEND', Configuration::get('CHANNABLE_DO_CRON_FROM_BACKEND') == '1' ? 1 : 0),
        ];
    }
     ...
     * @return array[]|mixed
     */
    public static function getMarketplaceIDAssignments()
    {
        $data = Configuration::get('CHANNABLE_MARKETPLACE_ID_ASSIGNMENTS');
        $json = json_decode($data, true);
        $struct = [
            's' => '',
            'g' => 0,
    ...
    public static function getCarrierShippingMethodAssignments()
    {
        $data = Configuration::get('CHANNABLE_CARRIER_SHIPPINGMETHOD_ASSIGNMENTS');
        $json = json_decode($data, true);
        $struct = [
            's' => '',
            'id_carrier' => 0,
    ...
    public static function getCountryShippingMethodAssignments()
    {
        $data = Configuration::get('CHANNABLE_COUNTRY_SHIPPINGMETHOD_ASSIGNMENTS');
        $json = json_decode($data, true);
        $struct = [
            'country_id' => '',
            'id_carrier' => '',
