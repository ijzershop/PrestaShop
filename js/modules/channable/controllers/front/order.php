    ...
    public function postProcess()
    {
        ...
        if (Tools::getValue('order')) {
            ...
            if ($_SERVER['REQUEST_METHOD'] === 'PUT') {
                ...
                $putData = Channable::fetchPhpInput();
                $putData = json_decode($putData, true);

                $memo = $putData['memo'] ?? null;
                ...
                if ($memo === null || $memo === '') {
                    ...
                    if ($order = $orders->getFirst()) {
                        ...
                        $append = sprintf("[%s] %s", $now, strip_tags($memo));

                        $order->note = trim((string)$order->note);
                        $order->note = $order->note === '' ? $append : ($order->note . "\n" . $append);

                        $order->update();

                        $jsonData = ['id' => $order->id, 'reference' => $order->reference, 'memo' => $order->note];
                    }
                }
                ...
                if ($order = $ordersCheck->getFirst()) {
                    ...
                    if (sizeof($error_array) == 0 && $valid) {
                        try {
                            ...
                            $customer_email = $postData->customer->email;
                            Hook::exec(
                                'channableEmailCreation',
                                [
                            ...
                            if (!$guest_checkout) {
                                ...
                                $customer = $customerObject->getByEmail(trim($customer_email));
                            ...
                            $marketplaceIDAssignments = Channable::getMarketplaceIDAssignments();
                            foreach ($marketplaceIDAssignments as $msa) {
                                if (trim($msa['s']) != '' && (int) $msa['g'] > 0) {
                                    if (strpos($postData->channel_id, $msa['s']) !== false) {
                                        $new_order_status = (int) $msa['g'];
                                    }
                                }
                            }
                            ...
                            if ((int) $customer->id == 0) {
                                ...
                                if (!empty($postData->shipment_method)) {
                                    $carrier_shipment_method_assignments = Configuration::get('CHANNABLE_CARRIER_SHIPPINGMETHOD_ASSIGNMENTS');
                                    $json_carrier_shipment_method_assignments = json_decode($carrier_shipment_method_assignments, true);
                                    if ($json_carrier_shipment_method_assignments != null && is_array($json_carrier_shipment_method_assignments)) {
                                        foreach ($json_carrier_shipment_method_assignments as $json_carrier_shipment_method_assignment) {
                                            if (isset($json_carrier_shipment_method_assignment['id_carrier']) && isset($json_carrier_shipment_method_assignment['s'])) {
                                                if ((int)$json_carrier_shipment_method_assignment['id_carrier'] > 0 && trim($json_carrier_shipment_method_assignment['s']) != '') {
                                                    if (strpos($postData->shipment_method, $json_carrier_shipment_method_assignment['s']) !== false) {
                                                        $id_carrier = $json_carrier_shipment_method_assignment['id_carrier'];
                                                    }
                                                }
                                            }
                                        }
                                ...
                                foreach ($postData->products as $product_key_post => $product) {
                                    ...
                                    foreach ($product_list as $product_key => $product_list_product) {
                                        ...
                                        if ($check_id == $posted_products[$product_key_post]) {
                                            $found_product = true;
                                            if (isset($product->tax_rate) && Configuration::get('CHANNABLE_USE_SUBMITTED_TAX_RATE') == '1') {
                                ...
                                if ((int)$delivery_address->id_country > 0) {
                                    $country_carrier_data = Configuration::get('CHANNABLE_COUNTRY_SHIPPINGMETHOD_ASSIGNMENTS');
                                    $json_country_carrier = json_decode($country_carrier_data, true);
                                    if ($json_country_carrier != null && is_array($json_country_carrier)) {
                                        foreach ($json_country_carrier as $country_carrier) {
                                            if (isset($country_carrier['country_id']) && $country_carrier['country_id'] == $delivery_address->id_country) {
                                                if (isset($country_carrier['id_carrier'])) {
                                                    $id_carrier = (int)$country_carrier['id_carrier'];
                                                }
                                            }
                                        }
                                    }
                                ...
                                if (!empty($postData->price->transaction_fee)) {
                                    if ($postData->price->transaction_fee > 0) {
                                        ...
                                        $orderDetail->product_name = 'Transaction Fee';
                                        $orderDetail->product_quantity           = 1;
                                        $orderDetail->product_quantity_in_stock  = 1;
                                        $orderDetail->product_quantity_refunded  = 0;
                                        $orderDetail->product_quantity_return    = 0;
                                        $orderDetail->product_quantity_reinjected = 0;
                                        $precision = defined('_PS_PRICE_DISPLAY_PRECISION_') ? (int) _PS_PRICE_DISPLAY_PRECISION_ : 2;
                                        $feeAmount = (float) Tools::ps_round($postData->price->transaction_fee, $precision);
                                        $orderDetail->product_price          = $feeAmount;
                                        $orderDetail->unit_price_tax_excl    = $feeAmount;
                                        $orderDetail->unit_price_tax_incl    = $feeAmount;
                                        $orderDetail->total_price_tax_excl   = $feeAmount;
                                        $orderDetail->total_price_tax_incl   = $feeAmount;

                                        $orderDetail->tax_name               = '';
