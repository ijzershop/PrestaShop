...
<div class="panel channable_configuration">
	...
	<div class="row">
		...
		<div class="col-xs-12 col-sm-6">
			<p class="channable_orderstates_config_head channable_top_marged">
				{l s='Add advanced stock module data to the feed:' mod='channable'}
			</p>
			<select name="enable_advanced_stock_fields" id="enable_advanced_stock_fields">
				<option value="0">No</option>
				<option value="1" {if Configuration::get('CHANNABLE_ADD_ADVANCED_STOCK') == '1'}selected{/if}>Yes</option>
			</select>
		</div>
		<div class="col-xs-12 col-sm-6">
			<p class="channable_orderstates_config_head channable_top_marged">
				{l s='Use tax rates sent by channable for new orders:' mod='channable'}
			</p>
			<select name="enable_use_submitted_tax_rate" id="enable_use_submitted_tax_rate">
				<option value="0">No</option>
				<option value="1" {if Configuration::get('CHANNABLE_USE_SUBMITTED_TAX_RATE') == '1'}selected{/if}>Yes</option>
			</select>
		</div>
	</div>
...
<div class="panel">
	...
	<br>
	<h3>{l s='Call the following URL via cronjob to build the feed cache:' mod='channable'}</h3>
	<p>
		{$product_cache_cron_url|replace:'%2C':','|escape:'html':'UTF-8'}<br />
		{l s='It is recommended to call the URL each minute.' mod='channable'}
	</p>
	<br>
	<h3>{l s='Call the following URL via cronjob to send product stock updates:' mod='channable'}</h3>
	<p>
		{$product_stock_update_cron_url|replace:'%2C':','|escape:'html':'UTF-8'}<br />
	</p>
</div>
...
<form method="post" action="{$form_url|escape:'html':'UTF-8'}">
	<div class="panel">
		<h3>{l s='Expert: Assign orders by ID to specific shipping status' mod='channable'}</h3>

		<div class="row">
			<div class="col-xs-12 col-md-6">
				<div id="id_shipping_fields">
					<div class="row">
						<div class="col-xs-6"><strong>{l s='String in ID contains:' mod='channable'}</strong></div>
						<div class="col-xs-6"><strong>{l s='Shipping status to set:' mod='channable'}</strong></div>
					</div>
					{foreach from=$marketplace_id_assignments item=msa key=nr}
						<div class="row">
							<div class="col-xs-6"><input type="text" name="msaid[{$nr|escape:'htmlall':'UTF-8'}][s]" value="{$msa.s|escape:'html':'UTF-8'}"></div>
							<div class="col-xs-6">
								<select name="msaid[{$nr|escape:'htmlall':'UTF-8'}][g]" class="msa_selector">
									<option value="0">---</option>
									{foreach from=$order_states item=os}
										<option value="{$os.id_order_state|escape:'html':'UTF-8'}" {if $os.id_order_state == $msa.g}selected{/if}>{$os.name|escape:'html':'UTF-8'}</option>
									{/foreach}
		...
		<div class="panel-footer">
			<button type="submit" value="1"	id="module_marketplace_id_assignment_form_submit_btn" name="submitChannableMarketplaceIDAssignmentModule" class="btn btn-default pull-right">
				<i class="process-icon-save"></i> {l s='Save' mod='channable'}
			</button>
		</div>
	</div>
...
<form method="post" action="{$form_url|escape:'html':'UTF-8'}">
	<div class="panel">
		<h3>{l s='Expert: Assigned carrier based on the Shipping Method' mod='channable'}</h3>

		<div class="row">
			<div class="col-xs-12 col-md-6">
				<div id="carriers_shipping_method_fields">
					<div class="row">
						<div class="col-xs-6"><strong>{l s='String in Shipping Method contains:' mod='channable'}</strong></div>
						<div class="col-xs-6"><strong>{l s='Assigned Carrier:' mod='channable'}</strong></div>
					</div>
					{foreach from=$carrier_shippingmethod_assignments item=csma key=nr}
						<div class="row">
							<div class="col-xs-6">
								<input type="text" name="csma[{$nr|escape:'htmlall':'UTF-8'}][s]" value="{$csma.s|escape:'html':'UTF-8'}">
							</div>
							<div class="col-xs-6">
								<select name="csma[{$nr|escape:'htmlall':'UTF-8'}][id_carrier]" class="carrier_selector">
									<option value="0">---</option>
									{foreach from=$carriers item=c}
										<option value="{$c.id_carrier|escape:'html':'UTF-8'}" {if $c.id_carrier == $csma.id_carrier}selected{/if}>{$c.name|escape:'html':'UTF-8'}</option>
									{/foreach}
								</select>
							</div>
						</div>
				...
				<p>
					{l s='To remove an assignment, just set the string empty.' mod='channable'}
				</p>
			</div>
		</div>
		<div class="panel-footer">
			<button type="submit" value="1"	id="module_carrier_shippingmethod_assignment_form_submit_btn" name="submitChannableCarrierShippingMethodAssignmentModule" class="btn btn-default pull-right">
				<i class="process-icon-save"></i> {l s='Save' mod='channable'}
			</button>
		</div>
	</div>
...
<form method="post" action="{$form_url|escape:'html':'UTF-8'}">
	<div class="panel">
		<h3>{l s='Expert: Assigned carrier based on delivery country' mod='channable'}</h3>

		<div class="row">
			<div class="col-xs-12 col-md-6">
				<div id="carriers_shipping_method_fields">
					<div class="row">
						<div class="col-xs-6"><strong>{l s='Delivery Country:' mod='channable'}</strong></div>
						<div class="col-xs-6"><strong>{l s='Assigned Carrier:' mod='channable'}</strong></div>
					</div>
					{foreach from=$country_shippingmethod_assignments item=ccma key=nr}
						<div class="row {if $nr >= 5}hidden_row_country{/if}" {if $nr >= 5}style="display:none;"{/if}>
							<div class="col-xs-6">
								<select name="ccma[{$nr|escape:'htmlall':'UTF-8'}][country_id]" class="country_entity_selector">
									<option value="0">---</option>
									{foreach from=$shop_countries item=country}
										<option value="{$country.id_country|escape:'htmlall':'UTF-8'}" {if $country.id_country == $ccma.country_id}selected{/if}>{$country.name|escape:'htmlall':'UTF-8'}</option>
									{/foreach}
								</select>
							</div>
							<div class="col-xs-6">
								<select name="ccma[{$nr|escape:'htmlall':'UTF-8'}][id_carrier]" class="carrier_selector">
									<option value="0">---</option>
									{foreach from=$carriers item=c}
										<option value="{$c.id_carrier|escape:'html':'UTF-8'}" {if $c.id_carrier == $ccma.id_carrier}selected{/if}>{$c.name|escape:'html':'UTF-8'}</option>
									{/foreach}
								</select>
							</div>
						</div>
				...
				<a href="JavaScript:void(0)" id="show_more_fields_country_link">
					{l s='Show more fields' mod='channable'}
				</a>
				<p>
					{l s='To remove an assignment, just set the country empty.' mod='channable'}
				</p>
			</div>
		</div>
		<div class="panel-footer">
			<button type="submit" value="1"	id="module_country_shippingmethod_assignment_form_submit_btn" name="submitChannableCountryCarrierShippingMethodAssignmentModule" class="btn btn-default pull-right">
				<i class="process-icon-save"></i> {l s='Save' mod='channable'}
			</button>
		</div>
	</div>
