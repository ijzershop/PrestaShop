...
//    $this->module_controller_url = __PS_BASE_URI__ . 'module/' . $this->name . '/AddModuleReviews';
  ...
  public function install()
  {
    ...
    if (count($this->_errors)) {
      return false;
				...
				|| !Configuration::updateValue('RECAPTCHA_DEBUG', 0)
  ...
  public function uninstall()
  {
    if (!Configuration::deleteByName('RECAPTCHA_PUBLIC_KEY') || !Configuration::deleteByName('RECAPTCHA_PRIVATE_KEY')
		|| !Configuration::deleteByName('RECAPTCHA_BADGE') || !Configuration::deleteByName('RECAPTCHA_DEBUG')
	)
      return false;
  ...
  public function postProcess()
  {
    if (Tools::isSubmit('SubmitCaptchaConfiguration')) {
      ...
      Configuration::updateValue('RECAPTCHA_DEBUG', (int)Tools::getValue('RECAPTCHA_DEBUG'));
    ...
    $fields_form = array(
      'form' => array(
        ...
        'input' => array(
          ...
          array(
            'type' => 'switch',
            'label' => $this->l('Debug reCaptcha'),
            'name' => 'RECAPTCHA_DEBUG',
            'hint' => $this->l('Allows you to display API response in case of error.'),
           'required' => true,
            'class' => 't',
            'is_bool' => true,
            'values' => array(
  ...
  public function hookDisplayHeader($params)
  {
    ...
    $this->context->controller->registerJavascript(
      'recaptcha',
      'https://www.google.com/recaptcha/api.js?onload=onloadCSCV3Callback&render=' . Configuration::get('RECAPTCHA_PUBLIC_KEY'),
      [
        'attributes' => 'async',
        'server' => 'remote'
      ]
  ...
  public function hookActionCSoftRecaptchaV3IsValid($param)
  {
    ...
    if (isset($verify) && $verify) {
      ...
      if ($decode['success'] === true && $decode['score'] >= $nscore) {
        ...
        $debugStr ='';
        if( (int)Configuration::get('RECAPTCHA_DEBUG')==1 ) {
          $debugStr = " API response: $response Score: $nscore";
        }
        $param['hookErrorMsg'] = $this->l('You have been detected as a robot, your form can\'t be send.' .  $debugStr);
      }
    } else {
      $param['hookErrorMsg'] = $this->l('An error occured, during API call.');
    }
    return $ret;
  }
