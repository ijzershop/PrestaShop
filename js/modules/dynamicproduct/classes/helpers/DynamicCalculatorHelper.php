...
class DynamicCalculatorHelper
{
    ...
    public function convertPrices($prices)
    {
        ...
        if (\Validate::isLoadedObject($currency) && (int) $currency->id !== $id_default_currency) {
            return $this->multiplyPrices($prices, (float) $currency->conversion_rate);
        }
    ...
    public function getQuantityDiscounts($id_product, $id_attribute, $calculated_prices)
    {
        ...
        $quantity_discounts = \SpecificPrice::getQuantityDiscounts(
            ...
            DynamicContext::getCurrencyId(),
            DynamicContext::getCountryId(),
            DynamicContext::getCustomerDefaultGroup() ?: (int) \Configuration::get('PS_CUSTOMER_GROUP'),
            $id_attribute,
            false,
            DynamicContext::getCustomerId()
        );
        ...
        foreach ($quantity_discounts as $quantity_discount) {
            ...
            $price_ttc = $this->module->calculator->applyTax($price_ht, $cart, false, $id_product);
            $price_ttc_nr = $this->module->calculator->applyTax($price_ht_nr, $cart, false, $id_product);
