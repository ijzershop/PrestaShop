...
class DynamicUploadHelper
{
    ...
    {
        $summary_helper = new SummaryHelper($this->module);
        \DynamicProduct::$logger->info('copyOrderFilesToKeep');
        $dest = $this->module->provider->getDataDir('upload_keep');
        \DynamicProduct::$logger->info('copyOrderFilesToKeep: dest ->' . $dest);
