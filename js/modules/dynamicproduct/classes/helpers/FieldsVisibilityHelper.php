...
class FieldsVisibilityHelper
{
    ...
    public function getFieldsVisibility($id_product, $id_attribute, $input_fields)
    {
        ...
        $visibility_helper = new FieldsVisibilityHelper($this->module);
        ...
        foreach ($input_fields as &$input_field) {
            ...
            if ($this->hasHiddenOptions($hidden_options, $options)) {
                ...
                if ($input_field->type === _DP_DROPDOWN_ && $input_field->isSelectedOptionExcluded()) {
                    ...
                    $input_field->removeExcludedOptionsFromSelection();
                } elseif ($input_field->type === _DP_FONT_ && $input_field->isSelectedOptionExcluded()) {
                    $input_field->selectFirstVisibleOption();
                }
            }
        }
    ...
    public function setHiddenItems($grouped_fields, $input_fields, array $visibility)
    {
        foreach ($grouped_fields as &$step) {
            $step['visible'] = 1;
            if (in_array($step['id'], $visibility['steps'])) {
                $step['visible'] = 0;
            }
            ...
            foreach ($step['groups'] as &$group) {
                ...
                $fields = $group['fields'];
                if (
                    in_array($group['id'], $visibility['groups'])
                    || in_array($group['id_source_group'], $visibility['groups'])
                    || (int) $group['id_control_field']
                    || !$step['visible']
                ) {
                    $group['visible'] = 0;
                }
                foreach ($fields as &$field) {
                    ...
                    if (
                        !$group['visible']
                        || $this->isHidden($visibility, $id_field)
                    ) {
                        $group['fields'][$id_field]['visible'] = 0;
                    }

                    if (isset($input_fields[$field['name']])) {
                        $input_fields[$field['name']]->visible = $group['fields'][$id_field]['visible'];
                    }

                    $options = $field['options'];
                    $hidden_options = $visibility['options'][$id_field] ?? [];
                    if ($this->hasHiddenOptions($hidden_options, $options)) {
                        foreach ($hidden_options as $hidden_option) {
                            $group['fields'][$id_field]['options'][$hidden_option]['visible'] = 0;
                        }
                    }
                }
            }
        ...
        return [$grouped_fields, $input_fields];
    }
}
