...
class DynamicInput extends DynamicObject
{
    ...
    public static function updateCartQuantities($cart, $add = false)
    {
        ...
        \DynamicProduct::$logger->info('Order has ' . count($dynamic_inputs) . ' dynamic inputs');
        foreach ($dynamic_inputs as $dynamic_input) {
            $dynamic_input->assignCartQuantity();
            $dynamic_quantity = $dynamic_input->dynamic_quantity * $dynamic_input->cart_quantity;
            \DynamicProduct::$logger->info('Dynamic input quantity', [
                'dynamic_input_id' => $dynamic_input->id,
                'cart_quantity' => $dynamic_input->cart_quantity,
                'dynamic_quantity' => $dynamic_input->dynamic_quantity,
                'total' => $dynamic_quantity,
            ]);
            if ($dynamic_quantity) {
                ...
                $depends_on_stock = !method_exists(\StockAvailable::class, 'dependsOnStock') || !\StockAvailable::dependsOnStock($dynamic_input->id_product);
                \DynamicProduct::$logger->info('Updating stock', [
                ...
                if ($cart_quantity && $depends_on_stock) {
    ...
    {
        $this->cart_quantity = self::getCustomizationQuantity($this->id_customization);
    }
    ...
    public function getInputFields($id_lang = null)
    {
        ...
        if (empty($this->input_fields)) {
            $legacy_input_fields = new LegacyInputFields($this->module);
    ...
    {
        $rows = [];
        $input_fields = DynamicInputField::getByIdInput($this->id, DynamicContext::getLanguageId());
    ...
    public function deleteFromCart()
    {
        ...
        if ($id_customization) {
            ...
            DynamicContext::getCart()->deleteProduct(
    ...
    public static function getCartMaxSizes(?\Cart $cart)
    {
        ...
        foreach ($products as $product) {
            $product = new \Product($product['id_product'], false, DynamicContext::getLanguageId());
    ...
    public static function getCustomizationQuantity($id_customization)
    {
        $sql = new \DbQuery();
        $sql->select('quantity');
    ...
    public function canDisplayWeight()
    {
        ...
        return $product_config->display_weight && (float) $this->weight;
    }
    ...
    public function getCSVLink()
    {
        return DynamicTools::addQueryToUrl(
            DynamicContext::getLink()->getAdminLink('DynamicProductCSV'),
            [
                'action' => 'download_csv',
                'id_input' => $this->id,
            ]
    ...
    {
        $input_fields = $this->getInputFields(DynamicContext::getLanguageId());
