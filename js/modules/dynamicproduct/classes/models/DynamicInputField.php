...
class DynamicInputField extends DynamicObject
{
    ...
    public $image_url;
    /** @noinspection PhpUnused used in tpls */
    public $thumb_url;
    public $image_size;
    ...
    {
        $display_value = null;
        if (in_array($this->type, [_DP_DROPDOWN_, _DP_RADIO_, _DP_THUMBNAILS_, _DP_FONT_])) {
    ...
    {
        $actual_value = $this->getValueForCalculation();
        $current_value = (float) $this->value;
    ...
    public function getValueForCalculation(array $input_fields = [])
    {
        ...
        if (!empty($input_fields) && !$this->duplicated) {
            ...
            if ($group && (int) $group['id_control_field']) {
                ...
                foreach ($matches as $match) {
                    $total += (float) $input_fields[$match]->getValueForCalculation();
                }
        ...
        if (is_array($this->selected_options) && count($this->selected_options)) {
            ...
            foreach ($this->selected_options as $selected_option) {
                if (isset($options[$selected_option])) {
                    $option_value = $options[$selected_option]['value'];
    ...
    public function getSecondaryValueForCalculation(array $input_fields = [])
    {
        ...
        if (!empty($input_fields) && !$this->duplicated) {
            ...
            if ($group && $group['id_control_field']) {
                ...
                $total = 0;
                foreach ($matches as $match) {
                    $total += (float) $input_fields[$match]->getSecondaryValueForCalculation();
                }
    ...
    /** @noinspection PhpUnused used in API */
    ...
    public static function getInputFieldsFromData(
        ...
        $fields_helper = new DynamicFieldsHelper($module);
        ...
        foreach ($fields as $field) {
            // load new fields first because the value and secondary of the original field depends on them
            if (in_array($field['name'], array_column($new_fields, 'name'))) {
                $input_fields[$field['name']] = self::getInputFieldFromData(
                    $field,
                    $id_lang,
                    $fields['changed']
                );
        ...
        foreach ($fields as $field) {
            if (!empty($field['name']) && !isset($input_fields[$field['name']])) {
                $input_fields[$field['name']] = self::getInputFieldFromData(
                    $field,
                    $id_lang,
                    $fields['changed'],
        ...
        if ($config->custom_calculation && count($calculation_items)) {
            ...
            $visibility_helper = new FieldsVisibilityHelper($module);
        ...
        $visibility_helper = new FieldsVisibilityHelper($module);
        [$grouped_fields, $input_fields] = $visibility_helper->setHiddenItems(
            $grouped_fields,
            $input_fields,
            $fields_visibility
        );
     ...
     * @return DynamicInputField
     */
    public static function getInputFieldFromData($field, $id_lang, $changed, array $input_fields = []): DynamicInputField
    {
        $input_field = InputFieldFactory::create($field['type']);
        $input_field->id_field = $field['id'] ?? 0;

        if (isset($field['duplicated']) && (int) $field['duplicated']) {
            $input_field->duplicated = 1;
        }

        $excluded_props = ['options'];
        foreach (self::$definition['fields'] as $prop => $field_info) {
            if (in_array($prop, $excluded_props)) {
                continue;
            }
            if (isset($field[$prop])) {
                if (!in_array($field_info['type'], [self::TYPE_STRING, self::TYPE_HTML])) {
                    $value = self::formatValue($field[$prop], $field_info['type']);
                } else {
                    $value = $field[$prop];
                }
        ...
        if (isset($field['selected_options'])) {
            ...
            $input_field->value = $input_field->getValueForCalculation($input_fields);
            $input_field->secondary_value = $input_field->getSecondaryValueForCalculation($input_fields);
    ...
    {
        return !$this->field['active'] || !(int) $this->visible || in_array($this->name, $this->skipped_names, true);
    }
