...
class DynamicStep extends DynamicObject
{
    ...
    public static function getRowsByProduct($id_product, $id_lang): array
    {
        ...
        $steps = \Db::getInstance()->executeS(
            'SELECT fg.*, fgl.*, fg.id_step as id FROM `' . _DB_PREFIX_ . 'dynamicproduct_step` fg
            LEFT JOIN `' . _DB_PREFIX_ . 'dynamicproduct_step_lang` fgl
            ON (fg.`id_step` = fgl.`id_step` AND fgl.`id_lang` = ' . (int) $id_lang . ')
            LEFT JOIN `' . _DB_PREFIX_ . 'dynamicproduct_product_step` pfg
            ON (fg.`id_step` = pfg.`id_step`)
