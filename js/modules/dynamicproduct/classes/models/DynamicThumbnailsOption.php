...
class DynamicThumbnailsOption extends DynamicObject
{
    ...
    public function copyImagesFrom($id_thumbnails_option)
    {
        $thumbnails_option = new self($id_thumbnails_option);

        $image = $thumbnails_option->getImage();
        if (is_file($image)) {
        ...
        $thumb = $thumbnails_option->getThumb();
        if (is_file($thumb)) {
