...
class IntervalFormula extends DynamicObject
{
    ...
    public static function getIntervalFormulas($condition_groups)
    {
        ...
        foreach ($interval_formulas as $formula) {
            $formulas["$formula->id_interval_field-$formula->id_interval_condition_group"] = $formula;
        }
    ...
    public static function get($id_interval_field, $id_condition_group)
    {
        $sql = new \DbQuery();
        $sql->select(self::$definition['primary']);
        $sql->from(self::$definition['table']);
        ...
        $interval_formula = new self($id);
        $interval_formula->id_interval_field = (int) $id_interval_field;
        $interval_formula->id_interval_condition_group = (int) $id_condition_group;
