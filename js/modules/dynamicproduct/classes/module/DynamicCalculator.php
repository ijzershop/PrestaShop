...
class DynamicCalculator
{
    ...
    public function assignProductPrices($product, $display_price, &$result)
    {
        ...
        $dynamic_calculator_helper = new DynamicCalculatorHelper($this->module);
        if ($dynamic_config->display_dynamic_price) {
            try {
                $cache_helper = new CachedCalculationHelper($this->module);
    ...
    {
        $inputs = DynamicInput::getInputsByIdCart($cart->id);
        $weight = 0;
        foreach ($inputs as $input) {
            $weight += $input->getWeight() * $input->cart_quantity;
        }
