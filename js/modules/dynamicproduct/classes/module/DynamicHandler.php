...
class DynamicHandler
{
    ...
    public function copyConfig(
        ...
        if ($copy_options['settings']) {
            $product_config = DynamicProductConfig::getByProduct($id_product_new);
            $product_config->delete();
            $product_config = DynamicProductConfig::getByProduct($id_product_old);
            $product_config->id = 0;
    ...
    public function clearConfig($id_product)
    {
        ...
        $product_config = new DynamicProductConfig($id_product);
        $product_config->delete();
    }
    ...
    public function exportConfig($id_product, $link_images): array
    {
        ...
        foreach ($product_fields as $product_field) {
            ...
            if (count($product_field->options)) {
                foreach ($product_field->options as $option) {
                    /**
                     * @var DynamicDropdownOption|DynamicFontOption $option_obj
                     */
                    $option_obj = FieldOptionHelper::getOptionInstance($option['id'], $product_field->type);
                    if ($option_obj) {
                        if (property_exists($option_obj, 'image') && $option_obj->image) {
                        ...
                        if (property_exists($option_obj, 'font') && $option_obj->font) {
                            $font_path = $option_obj->getFont();
                            if ($font_path) {
                                $field_values['options'][$option['id']]['font_data'] =
                                  $link_images ? $option_obj->getFontUrl() : DynamicTools::encodeFile($font_path);
                            }
                        }
        ...
        $grouped_visibility_values = DynamicTools::organizeDoubleBy('id_attribute', 'id_field', $fields_visibility_values, 'visible');

        $mapped_visibility_values = [];

        foreach ($grouped_visibility_values as $id_attribute => $visibility_values) {
    ...
    public function importConfig($id_product_new, $data): void
    {
        ...
        if (isset($data['fields'])) {
            ...
            foreach ($product_fields as $field) {
                ...
                $field->settings->initial_value = $field->value;
                $field->settings->description = $field->description;
                $field->settings->short_description = $field->short_description;
                $field->settings->placeholder = $field->placeholder;
                ...
                if ($options_class) {
                    /**
                     * @var DynamicDropdownOption[]|DynamicFontOption[] $options
                     */
                    ...
                    foreach ($options as $option) {
                        ...
                        if ($image_data) {
                            ...
                            if ($content) {
                                ...
                                if (!in_array($extension, $allowed_images)) {
                                    throw new \Exception('Invalid image format ' . $extension . ' for option ' . $option->id);
                                }
                        ...
                        if (property_exists($option, 'font')) {
                            $font_data = $option_data['font_data'] ?? null;
                            if ($font_data) {
                                $content = $link_images ?
                                  self::downloadLink($font_data) :
                                  DynamicTools::decodeData($font_data);
                                if ($content) {
                                    $extension = pathinfo($option->font, PATHINFO_EXTENSION);
                                    if (!in_array($extension, $allowed_fonts)) {
                                        throw new \Exception('Invalid font format ' . $extension . ' for option ' . $option->id);
                                    }
                                    $font = "$option->id.$extension";
                                    $option->font = $font;
                                    $font_path = $option->getPathForCreation('font');
        ...
        if (isset($data['config'])) {
            $product_config = DynamicProductConfig::getByProduct($id_product_new);
            $product_config->id_product = (int) $id_product_new;
            $product_config->setProps($data['config']);
            $product_config->save();
        }
    }
    ...
    public function addCustomField($id_product, $can_add = true)
    {
        \DynamicProduct::$logger->info('addCustomField', ['id_product' => $id_product, 'can_add' => $can_add]);
        ...
        \DynamicProduct::$logger->info('addCustomField row check', ['found' => (bool) $row]);

        if ($row && !$this->isReserved((int) $row['id_customization_field'])) {
            $id_customization_field = (int) $row['id_customization_field'];
            \DynamicProduct::$logger->info('addCustomField using existing', ['id_customization_field' => $id_customization_field]);
            ...
            if (!$customization_field) {
                \DynamicProduct::$logger->info('addCustomField field missing, deleting and retrying');
            ...
            if ((int) $customization_field['required'] !== (int) $product_config->required) {
                \DynamicProduct::$logger->info('addCustomField updating required', ['old' => $customization_field['required'], 'new' => $product_config->required]);
                ...
                if (!$can_add) {
                    \DynamicProduct::$logger->info('addCustomField can_add=false, returning early', ['id_customization_field' => $id_customization_field]);
            ...
            $this->reserveCustomField($id_customization_field);
            \DynamicProduct::$logger->info('addCustomField returning existing', ['id_customization_field' => $id_customization_field]);

            return $id_customization_field;
        }

        if (!$can_add) {
            \DynamicProduct::$logger->info('addCustomField can_add=false, no existing field');
        ...
        if (!$product_config->active) {
            \DynamicProduct::$logger->info('addCustomField product not active');
        ...
        $sql->from('customization_field');
        $sql->where('id_product = ' . (int) $id_product);
        $sql->where('type = ' . (int) \ProductCore::CUSTOMIZE_TEXTFIELD);
        $customization_fields = \Db::getInstance()->executeS($sql);
        if (count($customization_fields)) {
            \DynamicProduct::$logger->info('addCustomField found existing customization fields', ['count' => count($customization_fields)]);
            ...
            if (!$this->isReserved($id_customization_field)) {
                \DynamicProduct::$logger->info('addCustomField reusing unreserved field', ['id_customization_field' => $id_customization_field]);
                $data = [
                ...
                $this->updateCustomizableProp($id_product);
                $this->reserveCustomField($id_customization_field);
                \DynamicProduct::$logger->info('addCustomField returning reused field', ['id_customization_field' => $id_customization_field]);

                return $id_customization_field;
            }
        }

        \DynamicProduct::$logger->info('addCustomField creating new customization field');
        $labels = [];
        ...
        $this->updateCustomizableProp($id_product);
        $this->reserveCustomField($id_customization_field);
        \DynamicProduct::$logger->info('addCustomField returning new field', ['id_customization_field' => $id_customization_field]);
            ...
            if ($input_has_errors) {
                ...
                if ($cart) {
                    $cart->deleteProduct(
                        ...
                        $id_customization,
                        DynamicContext::getCartAddressDelivery()
                    );
                }
            }
        }
    ...
    public function addCart()
    {
        ...
        if (!\Validate::isLoadedObject($context->cart)) {
            ...
            $context->cart->id_currency = $this->module->provider->getCurrency();
        ...
        if ($id_customer) {
            ...
            $context->cart->id_address_delivery = $id_address;
            $context->cart->id_address_invoice = $id_address;
        }

        if (!(int) $context->cart->id_shop) {
            $context->cart->id_shop = DynamicContext::getShopId();
        }
        ...
        if ($context->cart->id) {
            ...
            $cookie->write();
            $context->cart->id = (int) $context->cart->id;
        }
    ...
    public function getOOSInputs()
    {
        ...
        $oos_inputs = [];
        $dynamic_inputs = DynamicInput::getInputsByIdCart($cart->id);
        foreach ($dynamic_inputs as $dynamic_input) {
    ...
    public function getDisabledOptions()
    {
        ...
        if (is_array($rows)) {
            foreach ($rows as $row) {
                ...
                $product = new \Product($dynamic_input->id_product, false, $id_lang);

                $class_name = $types[$type];
                /**
                 * @var DynamicDropdownOption $option
                 */
                $option = new $class_name($id_option, $id_lang);
     ...
     * @param int $id_group_new
     * @param int $id_step_new
     * @param bool $copy_position
     *
     * @return array
     */
    public function copyField($id_field, $id_product_new, $id_product_old = 0, $id_group_new = 0, $id_step_new = 0, $copy_position = false)
    {
        ...
        if ((int) $field->type === _DP_FONT_) {
            $font_options = DynamicFontOption::getFontOptionsByIdField($id_field_old);
            foreach ($font_options as $font_option) {
                $id_font_option = $font_option['id'];
                $new_font_option = new DynamicFontOption($id_font_option);
                $new_font_option->id_field = $id_field_new;
                $new_font_option->add();
                $options_new[$id_font_option] = (int) $new_font_option->id;
                $new_font_option->copyImagesFrom($id_font_option);
            }
        }
