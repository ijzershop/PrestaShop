...
class DynamicMedia
{
    ...
    public function addJS($js_uri, $params = [])
    {
        if (!$js_uri) {
            return;
        }
        ...
        if (method_exists($controller, 'registerJavascript')) {
            foreach ($js_uri as $js_file) {
                ...
                $controller->registerJavascript(
                    md5($js_file),
                    $js_file,
                    array_merge(['priority' => 100, 'position' => 'bottom'], $params)
                );
            }
        } else {
    ...
    public function addCSS($css_uri, $params = [])
    {
        $controller = DynamicContext::getController();
        if (!is_array($css_uri)) {
            $css_uri = [$css_uri];
        ...
        if (method_exists($controller, 'registerStylesheet')) {
            foreach ($css_uri as $css_file) {
                ...
                $controller->registerStylesheet(md5($css_file), $css_file, $params);
            }
        } else {
