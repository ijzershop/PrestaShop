import { At as fork, Ct as set$1, Jt as false_default, Tt as state, Yt as true_default, ct as tick$1, ft as effect_tracking, g as writable, kt as createSubscriber, lt as untrack$1, m as get$1, ot as get$2, r as onMount$1, st as settled, t as index_client_exports } from "./DUscB9kS.js";
import { i as SvelteKitError, n as HttpError, r as Redirect, t as base64_decode } from "./B_W7bg6n.js";
import { t as version } from "./sphe3lCN.js";
...
function fromStore(store) {
	let value = void 0;
	const subscribe = createSubscriber((update$1) => {
		let ran = false;
...
const base = globalThis.__sveltekit_jliom2?.base ?? "";
const assets = globalThis.__sveltekit_jliom2?.assets ?? base ?? "";
...
async function start(_app, _target, hydrate) {
	if (globalThis.__sveltekit_jliom2?.data) globalThis.__sveltekit_jliom2.data;
	...
	if (intent.id !== load_cache?.id) {
		...
		if (fork) {
			...
			lc.fork = lc.promise.then((result) => {
				if (lc === load_cache && result.type === "loaded") try {
					return fork(() => {
...
async function navigate({ type, url, popped, keepfocus, noscroll, replace_state, state: state$1 = {}, redirect_count = 0, nav_token = {}, accept = noop, block = noop, event }) {
	...
	if (started) {
		...
		const fork$1 = load_cache_fork && await load_cache_fork;
		if (fork$1) commit_promise = fork$1.commit();
		else {
			root.$set(navigation_result.props);
			update(navigation_result.props.page);
...
async function applyAction(result) {
	if (result.type === "error") await set_nearest_error_page(result.error, result.status);
	else if (result.type === "redirect") await _goto(result.location, { invalidateAll: true }, 0);
	else {
		...
		if (result.type === "success") reset_focus(page.url);
	}
}
async function set_nearest_error_page(error, status = 500) {
	...
	if (error_load) {
		...
		root.$set(navigation_result.props);
		update(navigation_result.props.page);
		tick().then(() => reset_focus(current.url));
	}
}
...
async function _hydrate(target$1, { status = 200, error, node_ids, params, route, server_route, data: server_data_nodes, form }) {
	...
	await initialize(result, target$1, hydrate);
}
...
export { goto as a, start as c, page as d, updated as f, beforeNavigate as i, stores as l, fromStore as m, app as n, invalidateAll as o, load_css as p, applyAction as r, onNavigate as s, afterNavigate as t, navigating as u };
