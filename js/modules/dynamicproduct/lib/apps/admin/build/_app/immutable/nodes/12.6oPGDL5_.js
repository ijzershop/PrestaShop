...
function _page($$anchor, $$props) {
	push($$props, true);
	const field_group_form = ($$anchor$1, $$arg0) => {
					...
					component(node_1, () => Card_content, ($$anchor$4, Card_Content) => {
						Card_Content($$anchor$4, {
							...
							children: ($$anchor$5, $$slotProps$1) => {
								...
								{
									let $0 = user_derived(() => _("Field group name"));
									let $1 = user_derived(() => get(id) ? $$props.data.field_groups[get(id)].name : "");
									Input(node_3, {
										get id() {
											return `name-${get(id) ?? ""}`;
										},
								...
								each(div_1, 21, () => $$props.data.languages, ({ id_lang, iso_code }) => id_lang, ($$anchor$6, $$item) => {
									...
									var node_5 = sibling(node_4, 2);
									{
										...
										let $1 = user_derived(() => _("Field group label"));
										let $2 = user_derived(() => get(id) ? $$props.data.field_groups[get(id)].label[id_lang()] : null);
										Input(node_5, {
											get id() {
												return `label-${id_lang() ?? ""}`;
											},
								...
								{
									let $0 = user_derived(() => get(id) ? $$props.data.field_groups[get(id)].show_label : false);
									Checkbox(node_6, {
										id: "show_label",
										name: "show_label",
										get checked() {
	...
	async function toggle_field_group(id) {
		set(status, `toggling-${id}`);
		const response = await form_action({
			route: page.route.id,
			action: "toggle_field_group",
			data: { id }
		});
		if (response.success) {
	...
	async function delete_field_group(id) {
		if (!confirm(_("Are you sure you want to delete this field group?"))) return;
				...
				component(first_child(fragment_11), () => Card_title, ($$anchor$3, Card_Title) => {
					Card_Title($$anchor$3, {
						children: ($$anchor$4, $$slotProps$1) => {
							next();
							var text_5 = text();
							template_effect(($0) => set_text(text_5, $0), [() => _("Field Groups Configuration")]);
							...
							component(sibling(node_17, 2), () => Table_body, ($$anchor$5, Table_Body) => {
								Table_Body($$anchor$5, {
									children: ($$anchor$6, $$slotProps$2) => {
										...
										each(first_child(fragment_22), 17, () => reorder($$props.data.field_groups), ({ id, name, label: group_label, show_label }) => id, ($$anchor$7, $$item) => {
											let id = () => get($$item).id;
											let name = () => get($$item).name;
											let group_label = () => get($$item).label;
															...
															component(node_29, () => Table_cell, ($$anchor$10, Table_Cell_2) => {
																Table_Cell_2($$anchor$10, {
																	children: ($$anchor$11, $$slotProps$4) => {
																		next();
																		var text_13 = text();
																		template_effect(($0$1) => set_text(text_13, $0$1), [() => label(group_label())]);
															...
															component(first_child(fragment_35), () => Table_cell, ($$anchor$11, Table_Cell_5) => {
																Table_Cell_5($$anchor$11, {
																	...
																	children: ($$anchor$12, $$slotProps$4) => {
																		var div_3 = root_38();
																		field_group_form(child(div_3), id);
																		reset(div_3);
			...
			children: ($$anchor$2, $$slotProps) => {
				...
				Button(node_39, {
					...
					children: ($$anchor$3, $$slotProps$1) => {
						...
						template_effect(($0) => set_text(text_14, ` ${$0 ?? ""}`), [() => _("Add a new field group")]);
