...
var root_10 = from_html(`<span class="font-mono text-wrap leading-6 text-left"><!></span>`);
					...
					Sheet_Content($$anchor$3, {
						...
						children: ($$anchor$4, $$slotProps$1) => {
										...
										component(node_3, () => Sheet_title, ($$anchor$7, Sheet_Title) => {
											Sheet_Title($$anchor$7, {
												children: ($$anchor$8, $$slotProps$3) => {
													next();
													var text$1 = text();
													template_effect(($0) => set_text(text$1, $0), [() => _("Add field formula")]);
							...
							each(div, 21, () => get(items), ({ id, formula }) => id, ($$anchor$5, $$item) => {
								...
								{
									let $0 = user_derived(() => $form().selected.includes(id()) ? `border-color: ${getItemColor(CalculationItemType.FIELD_FORMULA_ITEM)}` : "");
									Button($$anchor$5, {
										onclick: () => {
										...
										children: ($$anchor$6, $$slotProps$2) => {
											...
											var alternate_1 = ($$anchor$7) => {
												...
												var consequent_1 = ($$anchor$8) => {
													var span_1 = root_10();
													html(child(span_1), () => highlight(formula()));
													reset(span_1);
