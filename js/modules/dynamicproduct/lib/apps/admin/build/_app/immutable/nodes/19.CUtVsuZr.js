...
var root_8 = from_html(`<div class="flex items-center justify-center justify-self-end rounded-sm bg-gray-600 p-1 text-white"> </div>`);
var root_7 = from_html(`<div class="flex w-full justify-between gap-2"><div class="field-btn rounded-sm p-1"> </div> <!></div>`);
				...
				component(first_child(fragment_1), () => Sheet_content, ($$anchor$3, Sheet_Content) => {
					Sheet_Content($$anchor$3, {
						...
						children: ($$anchor$4, $$slotProps$1) => {
										...
										component(node_3, () => Sheet_title, ($$anchor$7, Sheet_Title) => {
											Sheet_Title($$anchor$7, {
												children: ($$anchor$8, $$slotProps$3) => {
													next();
													var text$1 = text();
													template_effect(($0) => set_text(text$1, $0), [() => _("Add grid")]);
							...
							each(div, 21, () => get(items), ({ id, id_field_target }) => id, ($$anchor$5, $$item) => {
								let id = () => get($$item).id;
								let id_field_target = () => get($$item).id_field_target;
								{
									let $0 = user_derived(() => $form().selected.includes(id()) ? `border-color: ${getItemColor(CalculationItemType.GRID_ITEM)}` : "");
									Button($$anchor$5, {
										onclick: () => {
										...
										children: ($$anchor$6, $$slotProps$2) => {
											...
											reset(div_1);
											template_effect(() => set_text(text_2, $$props.data.fields[id_field_target()]?.name ?? "--"));
