...
<div class="panel">
              ...
              {foreach $unregistered_hooks as $hook}
                <li>
                  <label for="id_hook_{$hook.id_hook|intval}">
                    <input
                            id="id_hook_{$hook.id_hook|intval}"
  ...
  <div class="panel-footer">
    <a
            href="{$module_link|escape:'htmlall':'UTF-8'}"
    ...
    <p class="help-block">{l s='Fix the backoffice order template to display the html summary correctly' mod='dynamicproduct'}</p>
      ...
      {else}
        <p style="margin-top: 1em;">
            {l s='List of fixes to apply:' mod='dynamicproduct'}
        </p>
        <ul>
            {foreach $fixes_needed as $fix}
              <li>
                <pre>{$fix.path|escape:'htmlall':'UTF-8'}</pre>
                  {if $fix.search && $fix.replace}
                    <div>
                      <span>{l s='Search:' mod='dynamicproduct'}</span>
                      <pre>{$fix.search|escape:'htmlall':'UTF-8'}</pre>
                      <span>{l s='Replace:' mod='dynamicproduct'}</span>
                      <pre>{$fix.replace|escape:'htmlall':'UTF-8'}</pre>
                    </div>
                  {/if}
              </li>
            {/foreach}
...
<div class="panel" id="dp-fix-templates">
  ...
  <div class="well">
    ...
    <p
            ...
            style="margin-top: 1em;">{l s='This is a one-time operation. New orders will be synced automatically.' mod='dynamicproduct'}</p>
  </div>
  ...
  <form class="dp_configuration_form" action="#dp_cleanup_panel" method="post">
    <div class="well">
      ...
      <div class="form-group">
        ...
        <input class="form-control" name="cron_key" value="{$config->cron_key|escape:'htmlall':'UTF-8'}" />
      </div>

      <div class="form-group">
        <label>{l s='Cron link' mod='dynamicproduct'}</label>
        <input
                class="form-control" name="cron_link" value="{$cron_link|escape:'htmlall':'UTF-8'}"
                data-link="{$cron_link|escape:'htmlall':'UTF-8'}" />
      </div>
    ...
    <div class="panel-footer">
      ...
      ><i class="process-icon-back"></i> {l s='Back' mod='dynamicproduct'}</a>
      <button
              type="submit" value="1" name="cleanup" class="btn btn-default btn-warning"
