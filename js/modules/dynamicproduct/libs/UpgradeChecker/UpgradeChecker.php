...
class UpgradeChecker
{
    ...
    public function display()
    {
        ...
        return DynamicContext::getSmarty()->fetch(dirname(__FILE__) . '/template.tpl');
    }
        ...
        foreach ($upgrades as $index => &$upgrade) {
            ...
            if (isset($upgrade['upgrade_function'][0])) {
                ...
                if (function_exists($check_function)) {
                    ...
                    if (!$upgrade['success']) {
                        \DynamicProduct::$logger->error('Upgrade failed', ['version' => $version]);
                    }
                } else {
    ...
    {
        $xml_files = glob($this->module->getDir() . 'config*.xml');
        \DynamicProduct::$logger->debug('Clearing module cache', ['files' => $xml_files]);
        foreach ($xml_files as $file) {
            $deleted = unlink($file);
            if (!$deleted) {
                \DynamicProduct::$logger->error('Failed to delete file', ['file' => $file]);
            } else {
                \DynamicProduct::$logger->debug('Deleted file', ['file' => $file]);
            }
        }
    }
