...
use MsThemeConfig\Core\Domain\Customer\CommandHandler\AddCustomerHandlerInterface;
...
final class AddCustomerHandler extends AbstractCustomerHandler implements AddCustomerHandlerInterface
{
     ...
     * @return CustomerId
     */
    public function __invoke(AddCustomerCommand $command)
    {
    ...
    private function assertCustomerWithGivenEmailDoesNotExist(Email $email)
    {
        ...
        if ($customer->id) {
            throw new DuplicateCustomerEmailException(
                $email,
                sprintf('Registered customer with email "%s" already exists', $email->getValue()),
