...
namespace MsThemeConfig\Adapter\Customer\CommandHandler;

use Customer;
use PrestaShop\PrestaShop\Adapter\Customer\CommandHandler\AbstractCustomerHandler;
use PrestaShop\PrestaShop\Core\Crypto\Hashing;
use MsThemeConfig\Core\Domain\Customer\Command\EditCustomerCommand;
use MsThemeConfig\Core\Domain\Customer\CommandHandler\EditCustomerHandlerInterface;
...
final class EditCustomerHandler extends AbstractCustomerHandler implements EditCustomerHandlerInterface
{
     ...
     * @return void
     */
    public function __invoke(EditCustomerCommand $command)
    {
        $this->handle($command);
    }
    ...
    private function updateCustomerWithCommandData(Customer $customer, EditCustomerCommand $command)
    {
        ...
        if (null !== $command->getInformerIdentification()) {
            $customer->informer_identification = $command->getInformerIdentification();
        }
    ...
    private function assertCustomerWithUpdatedEmailDoesNotExist(Customer $customer, EditCustomerCommand $command)
    {
        ...
        if ($customerByEmail->id) {
            throw new DuplicateCustomerEmailException(
                $command->getEmail(),
                sprintf('Registered customer with email "%s" already exists', $command->getEmail()->getValue()),
    ...
    private function assertCustomerCanAccessDefaultGroup(Customer $customer, EditCustomerCommand $command)
    {
        ...
        if (!in_array($defaultGroupId, $groupIds)) {
            throw new CustomerDefaultGroupAccessException(sprintf('Customer default group with id "%s" must be in access groups', $defaultGroupId));
        }
    }
}
