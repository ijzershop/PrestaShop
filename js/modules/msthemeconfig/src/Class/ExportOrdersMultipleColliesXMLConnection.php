 ...
 * Class ExportOrders.
 */
class ExportOrdersMultipleColliesXMLConnection
{
    ...
    public function __construct($id_order, array $collies = [])
    {
        ...
        $this->selectCarrier = \Carrier::resolveCarrierIdFromConfig((int) $this->getConfig('KOOPMANORDEREXPORT_SELECT_CARRIER'));

        $this->addedSelectCarrier = \Carrier::resolveCarrierIdFromConfig((int) $this->getConfig('KOOPMANORDEREXPORT_SELECT_ADDEDORDER_CARRIER'));
    ...
    private function processOrdersNew($orders, array $collies = []): void
    {
        ...
        foreach ($orders as $row) {
            ...
            $shippingTask->datum = $shippingDate;
    ...
    */
    ...
    public function dagafsluiting(): void
    {
        ...
        try {
            ...
            error_log(sprintf("Verzendlijst ophalen mislukt: %s (Code: %s)", $e->getMessage(), $e->getCode()));
        }
