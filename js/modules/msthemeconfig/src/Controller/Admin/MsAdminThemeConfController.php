...
use Configuration;
use PrestaShop\PrestaShop\Adapter\Address;
 ...
 */
class MsAdminThemeConfController extends PrestaShopAdminController
{
    ...
    {
        $this->idShop = (int) \Shop::getContextShopID();
        $this->idShopGroup = (int) \Shop::getContextShopGroupID();
    ...
    {
        // Redirect to the standard AdminModules configure URL for this module (compatible across PS versions)
        $link = $this->context && $this->context->link ? $this->context->link : null;
        if ($link) {
            $configureUrl = $link->getAdminLink('AdminModules', true, [], [
        ...
        // Fallback: legacy URL construction
        $fallback = 'index.php?controller=AdminModules&configure=' . urlencode($this->moduleName) . '&module_name=' . urlencode($this->moduleName);
