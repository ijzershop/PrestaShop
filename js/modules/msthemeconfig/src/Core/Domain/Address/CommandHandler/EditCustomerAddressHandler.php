...
namespace MsThemeConfig\Core\Domain\Address\CommandHandler;
...
/**
 * Custom EditCustomerAddressHandler for MsThemeConfig module
 */
final class EditCustomerAddressHandler extends AbstractAddressHandler implements EditCustomerAddressHandlerInterface
{
     ...
     * @return AddressId
     */
    public function __invoke(EditCustomerAddressCommand $command): AddressId
    {
    ...
    public function handle(EditCustomerAddressCommand $command): AddressId
    {
        try {
            ...
            // The address is used by an order so it is not edited directly, instead a copy is created
    ...
    private function getAddressFromCommand(EditCustomerAddressCommand $command): Address
    {
        ...
        if (null !== $command->getHouseNumber()) {
            $address->house_number = $command->getHouseNumber();
        }

        if (null !== $command->getHouseNumberExtension()) {
