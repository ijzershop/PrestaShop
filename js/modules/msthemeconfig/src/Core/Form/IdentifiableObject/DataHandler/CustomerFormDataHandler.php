...
namespace MsThemeConfig\Core\Form\IdentifiableObject\DataHandler;

use PrestaShop\PrestaShop\Core\CommandBus\CommandBusInterface;
use MsThemeConfig\Core\Domain\Customer\Command\AddCustomerCommand;
...
final class CustomerFormDataHandler implements FormDataHandlerInterface
{
    ...
    private DefaultGroupsProviderInterface $defaultGroupsProvider;
    ...
    private function buildCustomerAddCommandFromFormData(array $data)
    {
        ...
        $isGuest = (bool) ($data['is_guest'] ?? false);
        if ($isGuest) {
            $password = (new PasswordGenerator(new OpenSSL()))->generatePassword(16, 'RANDOM');
            $defaultGroupId = $this->defaultGroupsProvider->getGroups()->getGuestsGroup()->getId();
        ...
        $command = new AddCustomerCommand(
            ...
            (int) ($data['informer_identification'] ?? 0),
            $isGuest
        );
    ...
    private function buildCustomerEditCommand($customerId, array $data)
    {
        ...
        $command = (new EditCustomerCommand($customerId))
            ...
            ->setInformerIdentification((int) ($data['informer_identification'] ?? 0))
        ;
