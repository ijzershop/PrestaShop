		...
		{if $bulk_actions && $has_bulk_actions}
			<td class="row-selector text-center">
				{if isset($list_skip_actions.delete)}
					{if !($tr.$identifier|in_array:$list_skip_actions.delete)}
						<input type="checkbox" name="{$list_id}Box[]" value="{$tr.$identifier}"{if isset($checked_boxes) && $checked_boxes|is_array && $tr.$identifier|in_array:$checked_boxes} checked="checked"{/if} class="noborder" />
					{/if}
				{else}
					<input type="checkbox" name="{$list_id}Box[]" value="{$tr.$identifier}"{if isset($checked_boxes) && $checked_boxes|is_array && $tr.$identifier|in_array:$checked_boxes} checked="checked"{/if} class="noborder" />
				{/if}
		...
		{foreach $fields_display AS $key => $params}
            ...
            {/if}
					...
					{elseif isset($params.maxlength) && Tools::strlen($tr.$key) > $params.maxlength}
						<span title="{$tr.$key|strip_tags}">{$tr.$key|truncate:$params.maxlength:'...'}</span>
					{else}
...
{else}
	<tr>
		<td class="list-empty" colspan="{($fields_display|count)+1}">
