{* Copied from dynamicproduct/views/templates/front/product-form.tpl *}
<div id="dynamicproductform" style="display: contents;">
  <div class="col-12 col-md-8">
    <div class="form-group">
      <div class="input-group">
        <input type="number" min="1" step="1" class="form-control quantity" name="quantity" id="quantity" value="{if $input && $input->cart_quantity > 0}{$input->cart_quantity}{else}1{/if}">
        <div class="input-group-append">
          <div class="input-group-text" style="font-size: .9em;">stuk(s)</div>
        </div>
      </div>
    ...
    <div class="col-12">
      ...
      <input type="hidden" name="customer-id" id="customer-id" value="{$customer_id|default:0}">
      <input type="hidden" name="cart-id" id="cart-id" value="{$cart_id|default:0}">
      <input type="hidden" name="dp_cart" id="dp_cart" value="{$cart_id|default:0}">
              ...
              {foreach from=$group.fields item=input_field}
                ...
                {if $input_field->type === "2" || !empty($input_field->init)}
                  <input type="hidden" {if $field.settings.required}required="{$field.settings.required}"{/if}
                         ...
                         data-field="{$input_field|json_encode}"
                ...
                {if $type == 'input'}
                  ...
                  {if $input_field->label}
                    <label class="dp-input-field-{$field.name|escape:'htmlall':'UTF-8'} w-100" for="dp_{$field.name}">{ucfirst($input_field->label)|escape:'htmlall':'UTF-8'} <small class="float-right">(Min. {$field.settings.min}mm / Max.  {$field.settings.max}mm)</small></label>
                  {/if}
                  <div class="input-group mb-1">
                    <input type="number"
                      ...
                      {if $field.placeholder != null}placeholder="{$field.placeholder}"{/if}
                           ...
                           data-field="{$input_field|json_encode}"
                           aria-describedby="dp_symbol_{$input_field->id}"/>
                    {if isset($field.unit_displayed) && $field.unit_displayed && $field.id_unit > 0}
                      <div class="input-group-append">
...
{*                  {$input_field->displayValue()|escape:'htmlall':'UTF-8'}*}
                ...
                <strong>{l s='Weight' mod='dynamicproduct'}:</strong>
                  <span class="dp-weight">{$input_field->getWeight()|floatval}</span> {$weight_unit|default:''|escape:'htmlall':'UTF-8'}
              </span>
                {/if}
              {/foreach}
              {if $grouped_fields|count > 1}
                <br>
              {/if}
  ...
  <div class="add col-12">
    <button class="btn btn-success addToCartProductView w-100" data-dp-id-input="{if isset($dp_id_input)}{$dp_id_input}{/if}" data-product-id="{$product->id}" type="button">
      <i class="fasl fa-cart-shopping cart-shopping" data-product-id="{$product->id}"></i>
      <span class="d-none d-lg-inline"> {l s='Add to cart' d='Shop.Theme.Actions'} </span>
    </button>
  </div>
</div>

  {* END Dynamic product form *}
</div>
</div>
