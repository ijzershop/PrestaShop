							...
							{block name="label"}
								{if isset($input.label)}
									<label class="control-label col-lg-3{if isset($input.required) && $input.required && $input.type != 'radio'} required{/if}">
										{if isset($input.hint)}
										<span class="label-tooltip" data-toggle="tooltip" data-html="true" title="{if $input.hint|is_array}
							...
							{block name="field"}
											...
											<div class="form-control-static row">
												<div class="col-xs-6">
													...
													{foreach $input.options.query AS $option}
														{if is_object($option)}
															{if !($option->$input.options.id|in_array:$fields_value[$input.name])}
								...
								{elseif $input.type == 'checkbox'}
									{if isset($input.expand)}
										<a class="btn btn-default show_checkbox{if $input.expand.default|strtolower == 'hide'} hidden{/if}" href="#">
										...
										<a class="btn btn-default hide_checkbox{if $input.expand.default|strtolower == 'show'} hidden{/if}" href="#">
									...
									{foreach $input.values.query as $value}
										...
										<div class="checkbox{if isset($input.expand) && $input.expand.default|strtolower == 'show'} hidden{/if}">
											{strip}
