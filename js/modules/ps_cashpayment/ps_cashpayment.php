...
public function addOrderState($name)
    ...
    public function hookDisplayPaymentReturn($params)
    {
        ...
        if (
            ...
            $newState = (int) Configuration::get('BANK_CASH_COMPLETE_STATE');
            // Only change state if a different completion state is explicitly configured
            if ($newState > 0 && $newState !== (int) $params['order']->getCurrentState()) {
        ...
        $fields_form_customization = array(
            'form' => array(
                ...
                'input' => array(
                    ...
                    [
                        'type' => 'select',
                        'label' => $this->trans('Order state for payment complete', [], 'Modules.Cashpayment.Admin'),
                        'name' => 'BANK_CASH_COMPLETE_STATE',
                        'desc' => $this->trans('The state for when payment is complete', [], 'Modules.Cashpayment.Admin'),
                        'required' => false,
                        'options' => [
                            'query' => $orderStates,
                            'id' => 'id_order_state',
    ...
    public function getConfigFieldsValues()
    {
        ...
        $newState = Tools::getValue('BANK_CASH_COMPLETE_STATE', Configuration::get('BANK_CASH_COMPLETE_STATE'));
