...
class ps_creditpaymentAjaxModuleFrontController extends ModuleFrontController
{
    ...
    public function initContent()
    {
        ...
        if($is_balie_employee){
            ...
            $whereCustomer = 'WHERE cg.`id_group` = ' . $creditGroup->id . ' ';
        } elseif (in_array($creditGroup->id, (array) $this->context->customer->getGroups())) {
            $whereCustomer = 'WHERE c.`id_customer` = ' . $this->context->customer->id . ' ';
        ...
        $customersGroup = (int)Configuration::get('MSTHEMECONFIG_EMPLOYEE_CUSTOMER_CREDIT_GROUP',  Context::getContext()->language->id, Context::getContext()->shop->id_shop_group, Context::getContext()->shop->id);
