...
class Ps_CreditpaymentValidationModuleFrontController extends ModuleFrontController
{
	...
	public function postProcess()
	{
        ...
        if(Tools::getIsset('dynamic_data')){
            ...
            $this->context->cookie->on_credit_reference = str_replace(['|', '¤'], ' ', $data->on_credit_reference);
            $this->context->cookie->on_credit_buyer = str_replace(['|', '¤'], ' ', $data->on_credit_buyer);
        } else {
        ...
        $order_status_id = Configuration::get('BANK_CREDIT_COMPLETE_STATE', null, null, null);
        if (!$order_status_id) {
            $order_status_id = Configuration::get('PS_OS_BANKCREDIT', null, null, null);
        }
        if (!$order_status_id) {
            $order_status_id = Configuration::get('PS_OS_PAYMENT', null, null, null);
        }

        $this->validateOrder($cart->id, $order_status_id, $total, 'Op Rekening', NULL, array(), (int)$currency->id, false, $this->context->cookie->selected_customer_secure_key);
    ...
    public function validateOrder(
        ...
        if (Configuration::get('PS_TAX_ADDRESS_TYPE', null, null, null) == 'id_address_delivery') {
            $context_country = $this->context->country;
        }
        $order_status = new OrderState((int) $id_order_state, (int) $this->context->language->id);
        if (!Validate::isLoadedObject($order_status)) {
            $id_order_state = Configuration::get('PS_OS_BANKCREDIT', null, null, null);
            if (!$id_order_state) {
                $id_order_state = Configuration::get('PS_OS_PAYMENT', null, null, null);
            }
            $order_status = new OrderState((int) $id_order_state, (int) $this->context->language->id);
        }
...
//            }
            ...
            if ($order_status->logable && number_format($cart_total_paid, $this->context->currency->precision) != number_format($amount_paid, $this->context->currency->precision)) {
                $id_order_state = Configuration::get('PS_OS_ERROR', null, null, null);
            }
            ...
            foreach ($order_detail_list as $key => $order_detail) {
                ...
                if (isset($order->id)) {
                    ...
                    foreach ($order->product_list as $product) {
                        $price = Product::getPriceStatic((int) $product['id_product'], false, ($product['id_product_attribute'] ? (int) $product['id_product_attribute'] : null), 6, null, false, true, $product['cart_quantity'], false, (int) $order->id_customer, (int) $order->id_cart, (int) $order->{Configuration::get('PS_TAX_ADDRESS_TYPE', null, null, null)}, $specific_price, true, true, null, true, $product['id_customization']);
                    ...
                    if (Configuration::get('PS_STOCK_MANAGEMENT', null, null, null) &&
                        ($order_detail->getStockState() ||
                            $order_detail->product_quantity_in_stock < 0)) {
                        $history = new OrderHistory();
                        $history->id_order = (int) $order->id;
                        $history->changeIdOrderState(Configuration::get($order->hasBeenPaid() ? 'PS_OS_OUTOFSTOCK_PAID' : 'PS_OS_OUTOFSTOCK_UNPAID', null, null, null), $order, true);
                        $history->add();
                    }
                    ...
                    if (Configuration::get('PS_ADVANCED_STOCK_MANAGEMENT', null, null, null)) {
                    ...
                    (new StockManager())->updatePhysicalProductQuantity(
                        (int) $order->id_shop,
                        (int) Configuration::get('PS_OS_ERROR', null, null, null),
                        (int) Configuration::get('PS_OS_CANCELED', null, null, null),
                        null,
                        (int) $order->id
                    );
                } else {
    ...
    {
        $email_configuration = Configuration::get('PS_MAIL_TYPE', null, null, null);
    ...
    protected function createOrderFromCart(
        ...
        $amount_paid = !$dont_touch_amount ? Tools::ps_round((float) $amount_paid, $currency->precision) : $amount_paid;
        ...
        if (null !== $carrier && Validate::isLoadedObject($carrier)) {
            $order->carrier_tax_rate = $carrier->getTaxesRate(new Address((int) $cart->{Configuration::get('PS_TAX_ADDRESS_TYPE', null, null, null)}));
        }
        ...
        $order->total_paid_tax_excl = (float) Tools::ps_round((float) $cart->getOrderTotal(false, Cart::BOTH, $order->product_list, $carrierId), $currency->precision);
        $order->total_paid_tax_incl = (float) Tools::ps_round((float) $cart->getOrderTotal(true, Cart::BOTH, $order->product_list, $carrierId), $currency->precision);
        $order->total_paid = $order->total_paid_tax_incl;
        $order->round_mode = Configuration::get('PS_PRICE_ROUND_MODE', null, null, null);
        $order->round_type = Configuration::get('PS_ROUND_TYPE', null, null, null);
    ...
    protected function createOrderCartRules(
        ...
        $calculator->processCalculation($currency->precision);
        ...
        foreach ($cartRulesData as $cartRuleData) {
            ...
            $remainingValue = Tools::ps_round($remainingValue, $currency->precision);
