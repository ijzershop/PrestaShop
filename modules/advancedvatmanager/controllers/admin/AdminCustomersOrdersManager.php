...
class AdminCustomersOrdersManagerController extends ModuleAdminController
{
    ...
    public function renderList()
    {
        ...
        $total_currentYearIntracommunity = CustomersOrders::getTotalIntracommunity(false, date("Y").'-01-01 00:00:00', $today);
        $total_lastYearIntracommunity = CustomersOrders::getTotalIntracommunity(false, (date("Y")-1).'-01-01 00:00:00', (date("Y")-1).'-12-31 23:59:59');
        ...
        $tpl_statistics->assign(array(
            ...
            'total_intracommunity' => version_compare(_PS_VERSION_, '1.7.6.0', '>=')?$this->context->getCurrentLocale()->formatPrice(CustomersOrders::getTotalIntracommunity(false), $this->context->currency->iso_code):Tools::displayPrice(CustomersOrders::getTotalIntracommunity(false)),
        ...
        return $tpl_panel->fetch().$tpl_statistics->fetch().parent::renderList().$tpl_export_invoices->fetch().$tpl_orders_importation->fetch();
    }
    ...
    public function setMedia($isNewTheme = true)
    {
        ...
        $this->addCSS('/modules/advancedvatmanager/views/css/admin/AdminCustomersOrdersManager/orders_manager.css');
        $this->addJS('/modules/advancedvatmanager/views/js/admin/AdminCustomersOrdersManager/orders_manager.js');
    ...
    public function importOrders()
    {
        ...
        $sql = 'SELECT * FROM `' . _DB_PREFIX_ . 'orders` ord INNER JOIN `'._DB_PREFIX_.'address` address ON address.id_address = ord.id_address_delivery INNER JOIN `'._DB_PREFIX_.'country` country ON address.id_country = country.id_country WHERE address.id_country != '.(int)$local_country;
        $orders_found = false;
        $countries_iso = ValidationEngine::$european_countries_iso;
        if (Configuration::get('ADVANCEDVATMANAGER_BREXIT_ENABLED')) {
            $countries_iso = array_merge($countries_iso, ValidationEngine::$brexit_countries_iso);
        }
        if (Configuration::get('ADVANCEDVATMANAGER_VOEC_ENABLED')) {
            $countries_iso = array_merge($countries_iso, ValidationEngine::$voec_countries_iso);
        }
        if ($orders = Db::getInstance()->executeS($sql)) {
            foreach ($orders as $order) {
                if (in_array($order['iso_code'], $countries_iso) && !CustomersOrders::checkCustomerOrderExists($order['id_customer'], $order['id_order'])) {
                    $orders_found = true;
                    $noTax = $order['total_paid_tax_incl'] == $order['total_paid_tax_excl'];
                    $brexit = (bool)in_array(Country::getIsoById($order['id_country']), ValidationEngine::$brexit_countries_iso);
                    $voec = false;
                    if ((bool)in_array(Country::getIsoById($order['id_country']), ValidationEngine::$voec_countries_iso)) {
                        if ($order['vat_number'] != '') {
                    ...
                    Db::getInstance()->execute('INSERT INTO `' . _DB_PREFIX_ . 'advancedvatmanager_orders` (`id_order`, `id_shop`, `id_customer`, `notax`, `brexit`, `voec`, `invoice`, `date_upd`, `date_add`)
                    VALUES ('.(int)$order['id_order'].', '.(int)$order['id_shop'].', '.(int)$order['id_customer'].', '.(int)$noTax.', '.(int)$brexit.', '.(int)$voec.', '.($order['invoice_number']?(int)$order['invoice_number']:'""').', "'.pSQL($order['date_upd']).'", "'.pSQL($order['date_add']).'");');
                }
            }
    ...
    public function ajaxProcessGetTotalsByDates()
    {
        if (Tools::getValue('token')) {
            ...
            $total_intracommunity = version_compare(_PS_VERSION_, '1.7.6.0', '>=')?Tools::getContextLocale($this->context)->formatPrice(CustomersOrders::getTotalIntracommunity(false, $date_from, $date_to), $this->context->currency->iso_code):Tools::displayPrice(CustomersOrders::getTotalIntracommunity(false, $date_from, $date_to));
            $total_intracommunity_wt = version_compare(_PS_VERSION_, '1.7.6.0', '>=')?Tools::getContextLocale($this->context)->formatPrice(CustomersOrders::getTotalIntracommunity(true, $date_from, $date_to), $this->context->currency->iso_code):Tools::displayPrice(CustomersOrders::getTotalIntracommunity(true, $date_from, $date_to));
