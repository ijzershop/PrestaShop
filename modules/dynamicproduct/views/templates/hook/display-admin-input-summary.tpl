  ...
  {if count($grouped_fields)}
    {foreach from=$grouped_fields item=group}
      ...
      {foreach from=$group.fields item=input_field}
        ...
        {if $input_field->getTemplatePath()}
          {if $input_field->label}
            <span><strong>{$input_field->label|escape:'htmlall':'UTF-8'}:</strong>{include file=$input_field->getTemplatePath()}</span>
            <br/>
          {/if}
        {else}
          {if $input_field->label}
            <span><strong>{$input_field->label|escape:'htmlall':'UTF-8'}:</strong>{$input_field->getDynamicValue($input->input_fields)|escape:'htmlall':'UTF-8'}</span>
            <br/>
          {/if}
        {/if}
      {/foreach}
