...
export default function OrderLinesTable(): ReactElement<{}> {
    ...
    async function _ship(origLines: Array<IMollieOrderLine>): Promise<void> {
        ...
        render(<OrderLinesEditor lineType="shippable" translations={translations} lines={origLines}
                                 edited={newLines => lines = newLines}/>, reviewWrapper);
        ...
        if (input) {
            ...
            render(<ShipmentTrackingEditor checkButtons={checkSwalButton} config={config} translations={translations}
                                           edited={newTracking => updateTracking(newTracking)}/>, trackingWrapper);
    ...
    async function _refund(origLines: Array<IMollieOrderLine>): Promise<void> {
        ...
        render(<OrderLinesEditor lineType="refundable" translations={translations} lines={filteredLines}
                                 edited={newLines => lines = newLines}/>, reviewWrapper);
        let el: any = reviewWrapper.firstChild;
        const {default: swal} = await import(/* webpackPrefetch: true, webpackChunkName: "sweetalert" */ 'sweetalert') as never as { default: SweetAlert };
        let input = await swal({
    ...
    async function _cancel(origLines: Array<IMollieOrderLine>): Promise<void> {
        let lines = null;
        const reviewWrapper = document.createElement('DIV');
        render(<OrderLinesEditor lineType="cancelable" translations={translations} lines={origLines}
                                 edited={newLines => lines = newLines}/>, reviewWrapper);
        let el: any = reviewWrapper.firstChild;
            ...
            <table className={cx({
                ...
                {order.lines.map((line: IMollieOrderLine) => (
                    <tr key={line.id} style={{marginBottom: '100px'}}>
                        ...
                        <td>{line.status}</td>
                        {viewportWidth < 1390 &&
                        <td>{line.quantityShipped} / {line.quantityCanceled} / {line.quantityRefunded}</td>}
