 ...
 *
 */
class PriceModificationDataFactory implements GridDataFactoryInterface
{
    /**
    ...
    private function modifyRecords(array $records)
    {
        ...
        foreach ($records as $key => $record) {
            ...
            $records[$key]['id_store_product_price'] = Product::getPriceStatic(
                ...
                $records[$key]['id_store_product_name'] = '';
            $records[$key]['id_store_product_price'] = 0;
            }
            ...
            try {
                ...
                if($id_product > 0){
                    ...
                    $records[$key]['db_handelslengte'] = round($this->getProductFeatureValue($id_product, 32, 1) / 1000, 2);
                    $records[$key]['db_gewicht'] = round($this->getProductFeatureValue($id_product, 45, 1), 2);
                    $records[$key]['db_prijs'] = $this->formattedPrice(Product::getPriceStatic($id_product, false, null, 2));
                } else {
$records[$key]['db_handelslengte'] = 0;
                    $records[$key]['db_gewicht'] = 0;
                    $records[$key]['db_prijs'] = 0;
                }
