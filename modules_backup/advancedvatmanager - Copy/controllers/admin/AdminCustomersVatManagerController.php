...
class AdminCustomersVatManagerController extends ModuleAdminController
{
    ...
    public function __construct()
    {
        ...
        $this->bulk_actions = array(
                    ...
                    'validateAPI' => array('text' => $this->l('Validate VAT with VIES/GOV.UK API system',$this->controller_name), 'icon' => 'fas fa-user-cog','confirm' => $this->l('Would you like to validate VAT number for the selected customers?',$this->controller_name)),
                    'sendValidationEmail' => array('text' => $this->l('Send VAT validation email request',$this->controller_name), 'icon' => 'far fa-envelope','confirm' => $this->l('Would you like to send email to request validate VAT number for the selected customers?',$this->controller_name)),
    ...
    public function showValidateIcons($value)
    {
        if ($value == 1) {
            ...
            if (CustomersVAT::checkVATWithSystemFails($this->id_address)) {
                $content .= '<i style="font-size:13px;color:#fdc700;position:relative;right:2px;" class="fal fa-exclamation-triangle" data-toggle="tooltip" data-placement="top" title="'.$this->l('VAT number validated by skipping validation when the API system failed. Repeat API validation is required.', $this->controller_name).'"></i>';
            }
        }
        else if ($value == 0) {
    ...
    public function displaySendValidationEmailLink($token = null, $id)
	{
		...
		$tpl->assign(array(
				...
				'action' => $this->l('Send request validation email', $this->controller_name),
				'id' => $id,
		));
        ...
        if (Tools::getValue('token')) {
            ...
            if (!empty($vat_address)) {
                CustomersVAT::updateCompanyName($vat_address['id_address'], $vat_address['registered_company']);
                 $this->message['success'] = sprintf($this->l('The company name %s has been updated correctly in address ID#%s',$this->controller_name), $vat_address['registered_company'], $vat_address['id_address']);
            }
            die(json_encode(array('success' => true, 'message' => $this->getMessage())));
        }
        else {
