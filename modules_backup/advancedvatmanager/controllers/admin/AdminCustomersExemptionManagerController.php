...
class AdminCustomersExemptionManagerController extends ModuleAdminController
{
    ...
    public function __construct()
    {
        ...
        $this->bulk_actions = array(
            'delete' => array('text' => $this->l('Delete', $this->controller_name), 'icon' => 'far fa-trash-alt','confirm' => $this->l('Would you like to delete selected items from the list?',$this->controller_name))
        );
        ...
        $this->fields_list = array(
            'id_advancedvatmanager_customers_exemption' => array('title' => $this->l('ID',$this->controller_name), 'type' => 'text', 'align' => 'text-center', 'class' => 'fixed-width-xs'),
            ...
            'customer_name' => array('title' => $this->l('Customer name',$this->controller_name), 'type' => 'text', 'filter_key' => 'c!lastname', 'align' => 'text-center'),
            ...
            'active' => array('title' => $this->l('VAT exemption',$this->controller_name),'align' => 'center','type' => 'bool','orderby' => false,'filter_key' => 'a!active','class' => 'fixed-width-sm', 'callback' => 'showValidateIcons'),
    ...
    {
        if ($value == 1) {
            $content = '<i style="font-size:20px;color:#32db1d;" class="fal fa-badge-percent" data-toggle="tooltip" data-placement="top" title="'.$this->l('VAT exemption', $this->controller_name).'"></i>'; 
        }
        else if ($value == 0) {
            $content ='<i style="font-size:20px;color:#f92727;" class="fal fa-badge-percent" data-toggle="tooltip" data-placement="top" title="'.$this->l('Force VAT collection', $this->controller_name).'"></i>';     
        }
        return $content;
    }
    ...
    public function setMedia($isNewTheme = true)
    {
        ...
        Media::addJsDef(array(
            'ajax_url_avm_search_customers' => $this->context->link->getAdminLink($this->controller_name),
            'no_found' => $this->l('No customer found!',$this->controller_name)
        ));
    ...
    public function initPageHeaderToolbar()
    {
        if ($this->display != 'add') {
            $this->page_header_toolbar_btn['new'] = array(
                'href' => $this->context->link->getAdminLink($this->controller_name).'&addadvancedvatmanager_customers_exemption&token'.Tools::getAdminTokenLite($this->controller_name),
                'desc' => $this->l('Add customer',$this->controller_name),
            ...
            $this->page_header_toolbar_btn['back_to_list'] = array(
                ...
                'desc' => $this->l('Back to list',$this->controller_name),
    ...
    public function renderForm()
    {
        ...
        $this->fields_form = array(
            'legend' => array('title' => $this->l('Add/Edit customer for VAT force/exemption', $this->controller_name),
            ...
            'input' => array(
                array(
                    ...
                    'desc' => $this->l('Type customer ID, name or email to find results in live.', $this->controller_name),
                ),
                array(
                    'type' => 'select',
                    'label' => $this->l('Customer', $this->controller_name),
                    'name' => 'id_customer',
                    'desc' => $this->l('Select to add the customer. Only customers who have not been assigned to this list will be available for selection.', $this->controller_name),
                    'required' => false,
                    'options' => array(
                        'query' => $customers,
                        'id' => 'id',
                ...
                array(
                    'type' => 'switch',
                    'label' => $this->l('VAT option', $this->controller_name),
                    'name' => 'active',
                    'desc' => $this->l('Enable VAT exemption or force VAT collection for this customer.', $this->controller_name),
                    'required' => false,
                    'is_bool' => true,
                    'values' => array(
                        array(
                            'id' => 'active_on',
                            'value' => 1,
                            'label' => $this->l('VAT exemption', $this->controller_name)
                        ),
                        array(
                            'id' => 'active_off',
                            'value' => 0,
                            'label' => $this->l('Force VAT collection', $this->controller_name)
                        )
                    ),
                ),
            ),
        ...
        if (Shop::isFeatureActive()) {
            $this->fields_form['input'][] = array(
                'type' => 'shop',
                'label' => $this->l('Shop association', $this->controller_name),
                'hint' => $this->l('Select the shops which the VAT exemption will be applied for the customer.', $this->controller_name),
    ...
    {   
        ...
        $helper->module = $this;
        $this->toolbar_title = $this->l('Customer VAT Exemption list',$this->controller_name); // title
