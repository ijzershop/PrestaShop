...
class PreviewField extends DynamicField
{
    /** @var DynamicPreviewOption[] */
    public $options;

    public function getOptions()
    {
        return DynamicPreviewOption::getPreviewOptionsByIdField($this->id);
    }
    ...
    public function getInitialOptions($initial_value = null)
    {
        if (count($this->options)) {
            foreach ($this->options as $option) {
                if (is_numeric($option['value']) && (float) $option['value'] === (float) $this->init) {
                    return [$option['id']];
                }
            }
        }
