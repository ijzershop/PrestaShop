...
class DynamicCalculationItemsController extends ModuleAdminController
{
    /** @var DynamicProduct */
    public $module;
    public $action;
    ...
    {
        $this->respond([
            'calculation_items' => DynamicCalculationItem::getRowsByProduct($this->id_product),
        ]);
    }

    private function processInsertItems()
    {
        $items = (array) Tools::getValue('items', []);
        $type = Tools::getValue('type');

        foreach ($items as $id_item) {
            $calculation_item = new DynamicCalculationItem();
            $calculation_item->id_product = $this->id_product;
            $calculation_item->id_item = $id_item;
            $calculation_item->type = $type;
    ...
    private function processDeleteItem()
    {
        ...
        $calculation_item = new DynamicCalculationItem($id_item);
    ...
    private function processDeleteAllItems()
    {
        Db::getInstance()->delete(
            $this->module->name . '_calculation_item',
    ...
    private function processReorderItems()
    {
        $order = (array) Tools::getValue('order');
        foreach ($order as $position => $id_item) {
            $calculation_item = new DynamicCalculationItem((int) $id_item);
            $calculation_item->position = (int) $position + 1;
            $calculation_item->save();
        }
        $this->respond([
            'calculation_items' => DynamicCalculationItem::getRowsByProduct($this->id_product),
