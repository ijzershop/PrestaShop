    ...
    public function getImageLink($name, $ids, $type = null, $extension = 'jpg')
    {
        ...
        if (Module::isEnabled('wkwebp')
            ...
            if (Configuration::get('PS_REWRITING_SETTINGS')) {
                $theme = ((Shop::isFeatureActive() && file_exists(_PS_PRODUCT_IMG_DIR_ . Image::getImgFolderStatic($id) . $id . ($type ? '-' . $type : '') . '-' . (int) Context::getContext()->shop->theme_name . '.webp')) ? '-' . Context::getContext()->shop->theme_name : '');
