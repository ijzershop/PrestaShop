            ...
            if(!$this->checkIfActiveInShop($product, Context::getContext())){
                ...
                if($product->price_amount == 0 && class_exists('DynamicProductController')){
                    ...
                    if(array_key_exists('formatted_prices', $data) && array_key_exists('final_prices', $data) && array_key_exists('unit_prices', $data)){
                        // Use numeric values from final_prices, not formatted strings
                        $product->price_amount = $data['final_prices']['price_ttc'];
                        $product->regular_price_amount = $data['final_prices']['price_ttc_nr'];

                        // Format the price properly using PrestaShop's formatting
                        $product->price = Tools::displayPrice($data['final_prices']['price_ttc']);
                        $product->regular_price = Tools::displayPrice($data['final_prices']['price_ttc_nr']);

                        // Set unit price as numeric value
                        $product->unit_price = $data['final_prices']['price_ttc'];
                    }
                }
            }
        }
