    ...
    public function getContent()
    {
        ...
        foreach ($order_details as &$order_detail) {
            if (!is_null($order_detail['id_oi_offer'])) {
                ...
                if ($descProduct) {
                    $order_detail['product_desc_short'] = trim(str_replace('""','',reset($descProduct->description_short)));
                }
            }
            ...
            if ($order_detail['product_reference'] == Configuration::get('MSTHEMECONFIG_CUSTOM_PRODUCT_REFERENCE', $this->order->id_lang, $this->order->id_shop_group, $this->order->id_shop,'CP')) {
                $descProduct = new Product($order_detail['product_id']);
                if ($descProduct) {
                    $order_detail['product_desc_short'] = trim(str_replace('""','',reset($descProduct->description)));
                }
            }
