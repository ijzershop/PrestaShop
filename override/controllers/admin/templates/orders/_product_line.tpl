...
<tr class="product-line-row">
	...
	<td>
      ...
      <span class="productName h3">{AttributeGroup::stripSawCutModuleAttributeGroupName($product['product_name'], Context::getContext()->cookie->id_lang)}</span><br />
      {if (int)$product['id_oi_offer'] > 0}{$product['description_short'] nofilter}{/if}
	...
	<td class="partial_refund_fields current-edit" colspan="2" style="display:none; width: 250px; min-width: 250px;">
        ...
        <p class="help-block"><i class="icon-warning-sign"></i> {l s='(Max %amount_refundable% %tax_method%)' sprintf=[ '%amount_refundable%' => Context::getContext()->currentLocale->formatPrice(Tools::ps_round($amount_refundable, 2), 'EUR'), '%tax_method%' => $smarty.capture.TaxMethod] d='Admin.Orderscustomers.Help'}</p>
			</div>
		</div>
		{/if}
	</td>
