    ...
    <div class="row">
      ...
      <div class="col-2">
        <div class="dropdown" style="height:25px;margin-bottom:5px;">
          <button type="button" id="cart-product-row-toggle-{$product.id_product}" title="Bekijk, wijzig of verwijder product" class="d-block dropdown-toggle show shoppingcart-ellipsis text-dark" data-toggle="dropdown" aria-expanded="false"><i class="text-right align-top fa-2x fasl fa-ellipsis-v float-right"></i></button>
          <div class="dropdown-menu shoppingcart-dropdown-menu" aria-labelledby="cart-product-row-toggle-{$product.id_product}">
            <a href="{Context::getContext()->link->getProductLink($product)}" rel="noopener" target="_blank" class="dropdown-item"><i class="fasl fa-question-circle"></i> Bekijk product</a>
            <a href="#" class="dropdown-item changeCartProductQty"><i class="fasl fa-edit"></i> Wijzig aantal: {$product.quantity}</a>
            <a href="#" class="ajax_remove_button dropdown-item" rel="nofollow" data-href="{Context::getContext()->link->getRemoveFromCartURL($product.id_product, $product.id_product_attribute, $product.id_customization)}" data-link-action="remove-from-cart" title="{l s=" Remove" d="Shop.Theme.Actions" }"><i class="fasl fa-trash"></i>
              Verwijder
            </a>
          </div>
        </div>
