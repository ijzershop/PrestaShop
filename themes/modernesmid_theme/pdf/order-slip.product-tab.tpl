			...
			{foreach $order_details as $order_detail}
				...
				<tr class="product {$bgcolor_class}">
					<td class="product left">
						{AttributeGroup::stripSawCutModuleAttributeGroupName($order_detail.product_name)}
					</td>
					<td class="product center">
					...
					<td class="product right">
						{if $tax_excluded_display}
							- {displayPrice currency=$order->id_currency price=$order_detail.unit_price_tax_excl*$order_detail.product_quantity_refunded}
						{else}
							- {displayPrice currency=$order->id_currency price=$order_detail.unit_price_tax_incl*$order_detail.product_quantity_refunded}
						{/if}
					</td>
				</tr>
				...
				{foreach $order_detail.customizedDatas as $customizationPerAddress}
					{foreach $customizationPerAddress as $customizationId => $customization}
						<tr class="customization_data {$bgcolor_class}">
									...
									{foreach $customization.datas as $customization_types}
										{if isset($customization.datas[Product::CUSTOMIZE_TEXTFIELD]) && count($customization.datas[Product::CUSTOMIZE_TEXTFIELD]) > 0}
											{foreach $customization.datas[Product::CUSTOMIZE_TEXTFIELD] as $customization_infos}
												{if !empty($customization_infos.value)}
													<b>{$customization_infos.name}:</b> <i>{$customization_infos.value}</i>
											    {/if}
							...
							<td class="center" style="font-style:italic;">{$customization.quantity}</td>
							<td class="product"></td>
							<td class="product"></td>
						</tr>
					{/foreach}
