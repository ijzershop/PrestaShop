                  ...
                  <td class="align-middle">
                      {if Module::isEnabled('dynamicproduct') && Product::isDynamicProduct($product)}
                        ...
                        <span class="h6 product-title p-0" data-id-product="{$product.id_product}"><a
                            class="text-decoration-none text-black" href="{$product.url}">{$product.name}</a></span>
                        <br/>
                        <a class="help-text product-description-short text-decoration-none">{$dynamicProductConfig.indication_msg}</a>
                        <br/>
                        <span class="product-description-short">{$product.description_short nofilter}</span>
                      {else}
                        <span class="h6 product-title p-0" data-id-product="{$product.id_product}"><a
...
{*                                {/if}*}
                                <span class="inclusive-price"
                                      data-product-id="{$product.id_product}">{if Context::getContext()->cookie->price_vat_settings_incl === "true"}{$product.price}{else}{Context::getContext()->currentLocale->formatPrice((float)Product::getPriceStatic($product.id_product, false), 'EUR')}{/if}  </span>
                                <br>
                                {hook h='displayProductPriceBlock' product=$product type='unit_price'}
                                {hook h='displayProductPriceBlock' product=$product type='weight'}
                              {/if}
