  ...
  {block name='cart_summary_tax'}
      {if Context::getContext()->cart->getOrderTotal(false, Cart::ONLY_DISCOUNTS) > 0}
            <div class="cart-summary-line summary-total-discount">
              <span class="label sub">Korting</span><span class="value sub">{if Context::getContext()->is_counter_customer}{Context::getContext()->currentLocale->formatPrice(0-(float)Context::getContext()->cart->getOrderTotal(false, Cart::ONLY_DISCOUNTS_NO_CALCULATION), 'EUR')}{else}{Context::getContext()->currentLocale->formatPrice(0-(float)Context::getContext()->cart->getOrderTotal(false, Cart::ONLY_DISCOUNTS), 'EUR')}{/if}</span>
            </div>
      {/if}
    {if $cart.subtotals.tax > 0}
      <div class="cart-summary-line summary-total-tax">
        ...
        <span class="value sub" {if (float)Context::getContext()->cart->getOrderTotal(false, Cart::ONLY_PRODUCTS_WITHOUT_SHIPPING) == 0}style="border-bottom:1px solid #c0c0c0c0;"{/if}>
                 {if (Context::getContext()->is_counter_customer) && (float)Context::getContext()->cart->getOrderTotal(false, Cart::ONLY_REMAINDER_OF_DISCOUNTS) < 0}
