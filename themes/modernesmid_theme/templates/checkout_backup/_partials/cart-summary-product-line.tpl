  ...
  *}
  ...
  <table class="w-100">
    <tr>
      ...
      <td><span class="product-quantity font-weight-bold">{$product.quantity}x</span> <span class="product-name font-weight-bold">{$product.name}</span>
      </td>
      <td class="text-right">
    ...
    {foreach from=$customization.fields item="field"}<li>
      <tr>
        ...
        {else}
        <td>{if !Product::isDynamicProduct($product)}{$field.label}{/if}
        {if $field.type == 'text'}
